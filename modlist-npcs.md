...
- **Extended Bandit Dialogue** ([Nexus](https://www.nexusmods.com/skyrimspecialedition/mods/113168)) — Additional bandit dialogue lines beyond the jayserpa expansion. Adds commentary on faction presence, environment, and combat situations. Complements Bandit Lines Expansion. → `NPCs - Appearance`
- **Falmer Servant Lines Expansion** ([Nexus](https://www.nexusmods.com/skyrimspecialedition/mods/120995)) — 483 new voiced dialogue lines for Falmer Servants using professional voice talent. Makes servants act brain-washed/insane as lore suggests, with reactions to allies dying, environment, and enemies. By jayserpa and TheOscar0. ESL-flagged. Two optional plugins: increased spawn locations and servant outfit/weapon variety. → `NPCs - Appearance`
    - **Main file:** `Falmer Servant Lines Expansion` (the base mod). Optional `More Spawns` and `More Unique Servants` plugins available as separate downloads.
...
- **Khajiit Will Follow** ([Nexus](https://www.nexusmods.com/skyrimspecialedition/mods/2227)) — Four custom voiced Khajiit followers (Nanak, Bikhai, Ma'kara, Sug'ul) with individual backstories, personal quests, and inter-follower banter. Spawned across Skyrim, each with unique dialogue and commentary. ~8K endorsements. Compatible with NFF. → `NPCs - Followers`
    - **Main file:** `Khajiit Will Follow` (the base mod). Single main file.
