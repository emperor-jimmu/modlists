...
> NPC-specific content (Appearance, Population, Dialogue, Followers) → [NPCs](modlist-npcs.md).
...
- Lawless and OBIS are not interchangeable (maintenance/design tradeoffs differ). Same for Haugbui vs The Restless Dead (light adjustment vs major redesign). The Restless Dead is listed under `Creature Behavior Improvements` below as the undead-behavior baseline — verify that choice before locking Haugbui as an alternative here.
...
This subsection owns new creature types that add to the world's bestiary — Mihail Monsters and Animals variants, Morrowind-themed imports, and other authored creature packs that expand what the player can encounter. The goal is curated variety that makes exploration feel less predictable, not a raw creature count.
...
- **Imps - Mihail Monsters and Animals (SE-AE version)** ([Nexus](https://www.nexusmods.com/skyrimspecialedition/mods/33759)) — Low-level daedra variant. ESL-flagged. → `NPCs - Enemies & Creatures`
- **Giant Centipedes - Mihail Monsters and Animals (SE-AE version)** ([Nexus](https://www.nexusmods.com/skyrimspecialedition/mods/26192)) — Cave-dwelling hostile arthropods. ESL-flagged. → `NPCs - Enemies & Creatures`
- **Ogrim - Mihail Monsters and Animals (SE-AE version)** ([Nexus](https://www.nexusmods.com/skyrimspecialedition/mods/27243)) — High-level daedra variant. ESL-flagged. → `NPCs - Enemies & Creatures`
- **Ruinachs - Mihail Monsters and Animals (SE-AE version)** ([Nexus](https://www.nexusmods.com/skyrimspecialedition/mods/49543)) — Minotaur-like hostile creatures. ESL-flagged. → `NPCs - Enemies & Creatures`
...
- Mihail creature mods are each ESL-flagged and script-free, adding minimal plugin pressure. Test combat balance against the encounter-zone and difficulty baseline in `modlist-survival-combat.md`.
- Many Mihail creatures use minimal or no custom animation — they share vanilla skeleton rigs. Verify animation compatibility with creature-animation mods from `modlist-animations.md`.
- Morrowind Creatures and Ogres add more complex creatures. Verify Hunterborn compatibility (creature-harvest patching) and LoTD display integration if they drop unique loot.
- The Dragonborn's Bestiary (in `modlist-world-content.md`) can track all new creature types for the completionist angle.
