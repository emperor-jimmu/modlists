...
#mod-entry(
  ...
  impact: "Raises the difficulty of K2's signature resource by *postponing* ground imersite mining: the mod locks the Nauvis imersite drill behind the advanced tech card, so until that late research you cannot mine imersite on Nauvis at all. Instead you must bootstrap it from space — the mod adds an imersite asteroid type encountered en route to Aquilo and in the belt between Aquilo and the inner planets (a pre-Aquilo technology), with trace amounts also in Fulgora orbit. Ground mining becomes the later, second source once the advanced tech card unlocks. K2SO's own page describes the effect as pushing imersite mining later, toward Aquilo.",
  notes: "By snakeru; 6.4K downloads. Recommended by the K2SO author on the Krastorio 2 Spaced Out page. Power-spike verdict: a *challenge-raiser*, not a handout — it delays a resource that would otherwise be ground-mined early and forces a space bootstrap, so it passes the power-spike test cleanly (it gives nothing away; it takes ground access away and gates it behind advanced research).",
)
