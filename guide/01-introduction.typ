...
overpowered items are included, and no mod conflicts with another.
Where multiple mods serve the same function, only the best-maintained,
most modern option is chosen.
...
Every mod in this guide adheres to a core set of principles:

#callout("dependency")[
  *No cheats.* Mods that grant infinite resources, skip gameplay,
  reveal hidden information, or trivialize combat are excluded. Quality
  of life improvements that reduce tedium without removing challenge
  are welcome.
]

- *MELE-compatible only.* Every mod is verified to work with Mass Effect
  Legendary Edition (2021+). Original trilogy mods are never included.
- *No conflicts.* Each mod pair has been checked for compatibility.
  Known conflicts are documented in the Conflicts chapter.
- *No redundancy.* Where multiple mods cover the same ground, only the
  best-maintained option is chosen.
- *Modern, maintained mods preferred.* Mods updated within the last two
  years are prioritized for maximum compatibility with the current game
  version and modding toolchain.
