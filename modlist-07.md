...
- The subsection preserves room for the final `Weather severity and seasonal feel` decision and respects graphics/survival ownership boundaries already established elsewhere.

## Weather Severity And Seasonal Feel

### Core Idea

This subsection owns how forcefully weather should shape the feel of travel across Skyrim: how hard storms press on movement, how distinct seasonal mood changes should feel, and whether the climate identity of the world should lean toward readability, drama, or harsher atmospheric weight.

It does not create a second independent weather stack alongside `modlist-02.md`. Instead, it frames which already-researched weather direction best supports `Elder Wilds` as a world-feel decision.

### Suggested Options

- Readable modern travel route: use the already-researched `Azurite III CS` direction from `modlist-02.md` if the main goal is broad travel readability, clean atmospheric identity, and weather that supports exploration without overloading visibility.
- Gameplay-clarity weather route: use the already-researched `Raid Weathers CS` direction from `modlist-02.md` if the list wants weather severity to feel meaningful while still prioritizing forest, combat, and navigation clarity.
- Harsher classic-atmosphere route: use the already-researched `Obsidian Weathers and Seasons` direction from `modlist-02.md` as the more classic mood-forward comparison if the project wants travel and seasonality to feel more severe and austere.

### Recommendation

Start with the readable modern travel route.

The `Azurite III CS` direction is the strongest first-pass baseline because it fits the wider `Elder Wilds` approach already emerging across `modlist-07.md`: large-feeling travel, readable exploration, stronger night identity, and environmental atmosphere that supports movement rather than constantly obscuring it.

Keep `Raid Weathers CS` as the meaningful comparison if the main question is not overall beauty but how well weather severity and travel clarity can coexist during ordinary play.

Keep `Obsidian Weathers and Seasons` as the deliberate harsher classic branch if the project wants a stronger sense of austere climate identity and more severe outdoor mood than the cleaner CS-first routes provide.

### Why This Is The Right Baseline

Section 07 should answer what weather feels like to travel through, not restart the full graphics-weather debate already documented in `modlist-02.md`.

`Azurite III CS` fits this subsection well because it supports a cleaner, more readable travel experience while still giving the world a distinct atmospheric identity. That makes it a strong match for a modlist that already values third-person readability, believable travel scale, and environmental mood that does not constantly sabotage movement.

`Raid Weathers CS` matters because it is the clearer branch for players who want weather pressure and gameplay readability to stay tightly linked.

`Obsidian Weathers and Seasons` still matters as the classic harsher-weather comparison, especially because its mood and seasonal character are part of why it remains memorable, but it should stay the deliberate more-austere branch rather than the automatic baseline.

### Working Guidance For Elder Wilds

- Judge this subsection through ordinary travel across holds, mountain passes, forests, tundra, coastlines, and settlement approaches in different conditions: clear weather, fog, rain, storms, snow, dawn, dusk, and night.
- Reuse the weather shortlist already established in `modlist-02.md` instead of creating a second disconnected weather recommendation set here.
- Treat `Azurite III CS` as the first weather direction to test for overall travel readability and broad environmental consistency.
- Compare `Raid Weathers CS` if the bigger question is how much atmospheric pressure the world can carry without reducing navigation and traversal readability too much.
- Compare `Obsidian Weathers and Seasons` only if the project wants a stronger classic severity and more austere outdoor identity than the cleaner CS-native route provides.
- Preserve boundaries with graphics-side lighting/shader/weather ownership in `modlist-02.md`, survival-side weather pressure in `modlist-05.md`, and the already-written night-visibility subsection directly above.

### What Elder Wilds Should Prioritize

- Weather that makes travel feel larger and more atmospheric without constantly flattening route readability.
- A clear distinction between a cleaner readable travel-weather route, a gameplay-clarity route, and a harsher classic atmosphere route.
- Seasonal and storm identity that feels noticeable in play rather than only in screenshots.
- Coherence with the graphics stack already being built around Community Shaders, lighting, terrain, trees, and long-distance travel vistas.

### Research Tasks

- Re-check `Azurite III CS`, `Raid Weathers CS`, and `Obsidian Weathers and Seasons` specifically through the world-feel lens rather than only through screenshot comparison.
- Test which route best supports long-distance travel, city approaches, wilderness crossings, and nighttime movement once the current section-07 choices are considered together.
- Check whether the harsher weather routes create compelling travel pressure or just reduce readability too often.
- Revisit whether explicit seasonal feel is strong enough with the chosen weather direction to justify the already-listed `Seasonal Calendar` companion from earlier in this file.
- Record whether the chosen direction creates any new pressure on lighting, water, road visibility, wildlife readability, or survival-side weather systems.

### Risks To Check

- Quietly redoing the entire graphics weather decision here instead of keeping this subsection focused on world feel.
- Choosing the most dramatic weather route because it looks impressive in still images while making travel less enjoyable in motion.
- Making storms, fog, or snow so heavy that scale is expressed mostly through lost readability.
- Letting this subsection drift into survival weather pressure or night-visibility ownership that already belongs elsewhere.
- Treating seasonal language as meaningful even if the chosen weather route does not make seasonal shifts noticeable enough in play.

### Acceptance Criteria

- One clearly preferred weather direction is identified as the best fit for section-07 travel mood and seasonal feel, while reusing the shortlist already established in `modlist-02.md`.
- The distinction between a cleaner readable route, a gameplay-clarity route, and a harsher classic-atmosphere route is explicit.
- The chosen direction makes weather feel meaningful during travel without consistently undermining navigation or third-person readability.
- `modlist-07.md` can be considered fully expanded once this subsection is complete and weather ownership remains cleanly aligned with the graphics and survival sections.
