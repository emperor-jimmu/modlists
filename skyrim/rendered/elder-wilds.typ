...
*SkyPatcher* (by Zzyxzz, v7.0.0) is a runtime patcher built on SKSE and CommonLibSSE that modifies game data — NPCs, weapons, armors, leveled lists, formlists, containers, and more — from INI-style configuration files instead of traditional ESP plugins. It is the Skyrim port of #emph[RobCo Patcher] for Fallout 4, and it was originally created for #emph[SkyValor].
...
#table(
  ...
  [Rain wetness, puddles, raindrop splash/ripple VFX, shore wetness. 9.8K endorsements, v3.1.0.],
  [#link("https://www.nexusmods.com/skyrimspecialedition/mods/179371")[HDR]],
  [Native HDR10 output, configurable peak brightness (up to 10,000 nits), paper-white slider, BT.2020 colour space. *Requires an HDR-capable monitor.* Disable Windows Auto HDR and NVIDIA RTX HDR. 970 endorsements, v1.0.2.],
  ...
  [Soft ambient sky occlusion. Pairs cleanly with the conservative tier for an authentic grim-dark look.],
  ...
  [DLSS 4 / FSR 3.1 upscaling with FSR Frame Generation. The CS-team replacement for the older Skyrim Upscaler (80343). Requires SSE Display Tweaks. 6.4K endorsements, v1.4.0.],
  [#link("https://www.nexusmods.com/skyrimspecialedition/mods/179824")[Effects 11]],
  [ENB preset compatibility layer via FX11 framework. Experimental — evaluate before locking.],
)
...
Landscape and rock PBR overrides (TomatoRim PBR Landscapes, Enhanced Rocks and Mountains) are decided in → @graphics-terrain-terrain--flora. PGPatcher applies mesh patches across all separators — re-run once ALL PBR texture mods (Textures, Terrain, Characters, Lighting) are stable.
...
- *Tomato's Solitude Remastered - PBR-CPM* (#link("https://www.nexusmods.com/skyrimspecialedition/mods/181707")[Nexus]) — Solitude PBR override. Complements the VPBR base with hand-authored Solitude textures by Tomatokillz (same author as TomatoRim PBR Landscapes, PBR Farmhouses 2.0, Whiterun Remake — all locked baselines). FOMOD installer — choose the PBR branch, 2K recommended. Compatible with Grand Solitude (baseline) and Window Shadows Ultimate (baseline). Requires CS core + PGPatcher. Install after VPBR, before running PGPatcher.
...
- Landscape additions are owned by → @graphics-terrain-terrain--flora — TomatoRim PBR Landscapes AIO (177621) and Enhanced Rocks and Mountains (121336) are locked there; Cathedralist's PBR Landscape (137333) is rejected. No open landscape candidates belong in this section.
...
#table(
  ...
  [Higher-polygon mesh replacer for common objects. Adds geometry detail; evaluate vs conflict cost.],
)
...
#table(
  ...
  [Higher-res diffuse/normal replacer for common clutter. May be redundant with VPBR's clutter coverage — evaluate side-by-side.],
)
...
#table(
  ...
  [HD mushroom textures, 2K. Cathedral series. AI-Generated tag — quality eval per curation policy.],
  ...
  [Custom gargoyle skeleton replacer. Fits grim-dark pillar.],
...
== Open Research
<community-shaders--textures-open-research>
...
#table(
  ...
  [Fire],
  [140K+ endorsements. Install after main lighting overhauls. CS particle-light support built in. → `Graphics - Lighting`],
...
#table(
  ...
  [CS-native ground-up build. DALC, moon phase lighting, physical sky approximation. 526 endorsements. Nights reported as very dark — test before adopting.],
...
#table(
  ...
  [Rolling morning fogs over lakes/rivers (dawn windows, MCM-tunable). Adds new atmospheric fog only — compatible with everything except `Interior Floating Fog Remover`. *Verify fog density in third-person readability (same gate as Mists of Tamriel) and Azurite III CS fog overhead before locking.*],
)
...
#table(
  ...
  [*Locked single-pass projected snow baseline.* Replaces Simplicity of Snow (double-pass, rejected — incompatible with PBR landscapes). Install the v3.6.0 main file.],
...
#table(
  ...
  [Rejected — depends on Seasonal Landscapes which is rejected.],
)
...
#table(
  ...
  [*Baseline.* Regional tree variety per hold — natural distribution. Fits "Living the world" with varied forest biomes.],
...
#table(
  ...
  [Adds FSMP cloth physics to vanilla College mage robes (Apprentice, Journeyman, Archmage, CC Vagrant). Compatible with CBBE, 3BA, HIMBO. Requires FSMP (baseline). 567 endorsements, v1.01 (Jun 2026). Complemented by Clutter-free HIMBO College Robes (133675) for BodySlide zap projects.],
  [#link("https://www.nexusmods.com/skyrimspecialedition/mods/128173")[Mage Clothing Expansion]],
  [25 new lore-friendly mage robes (9 male / 9 female + darker variants + hood), crafted from vanilla assets. Craftable at tanning rack (3 leather). SPID distribution to College NPCs available. Prebuilt HIMBO + CBBE Curvy meshes. ESL-flagged. 7,093 endorsements, v2.0.1 (Sep 2024).],
  ...
  [BodySlide zap projects for HIMBO college robes + HDT-SMP CMR + MCE (satchel/pouch removal, sleeve/pants cuffs, collar zap, fur trim zap). Also fixes vertex scrunching on HIMBO's stock Archmage robes. FOMOD. Requires HDT-SMP College Mage Robes (183045) + MCE (128173). 68 endorsements, v1.2.4 (Jun 2026).],
)
...
#table(
  ...
  [Add-on],
  [#1, #4],
  [SKSE plugin (powerofthree): vampires cast no shadow in sunlight (humanoid + Lord forms). Zero records, compatible with every mesh/vampire mod. Visual immersion fix — aligns sun behavior with vampiric lore. v1.2.2 supports AE 1.6.1170.],
...
#table(
  ...
  [Framework],
  [all],
  [SKSE runtime dialogue-replacement framework (v1.4.1). Needed by the Simple Inn Bath dialogue expansion below; also reusable for future non-AI dialogue mods.],
  [#link("https://www.nexusmods.com/skyrimspecialedition/mods/49014")[Simple Inn Bath]],
  [Add-on],
  [all],
  [Bathhouse access at inns via innkeeper dialogue — minimal cell edits, high compatibility. *Confirmed working with the locked `Candlehearth` inn baseline (→ `modlist-world-content.md`, Inns) — no patches needed* (acts via dialogue/faction, not cell edits).],
  [#link("https://www.nexusmods.com/skyrimspecialedition/mods/158544")[Simple Inn Bath - Dialogue Expansion - DDR]],
  [Add-on],
  [all],
  [DDR-based voiced dialogue lines for Simple Inn Bath (~80 lines: spliced innkeeper + bathing dialogue). *Requires DDR (135618) + Simple Inn Bath*. Carries an *AI-Generated Content* tag — quality-eval per policy; *user-approved* (same gate precedent as Autumngate's xVASynth add-on).],
  [#link("https://www.nexusmods.com/skyrimspecialedition/mods/104314")[Simple Inn Bath for New Lands — SPID]],
  [Add-on],
  [all],
  [SPID patches granting Simple Inn Bath dialogue to Bruma, Falskaar, Skyrim - Obscure's College of Winterhold, Beyond Reach, Wyrmstooth, and Midwood Isle innkeepers so you bathe at those new-land inns. Requires SPID (baseline).],
  [Minimalist route],
  [Alternative],
  [all],
...
#table(
  ...
  [Alternative],
  [all],
  [Lighter eating/drinking animation replacement. v1.9.4, 11,210 endorsements. *Competes with the locked `Animated Eating Redux` baseline on the same animation events — choose one.*],
)
...
#table(
  ...
  [Alternative],
  [#1, #4],
  [Nemesis system — respawn with debuffs, enemies gain name/buffs/presence. Rejected due to script debt at this list's complexity.],
)
...
#table(
  ...
  [Add-on],
  [all],
  [Fully-voiced follower/spouse/adopted-kids appreciation via dialogue + hug animation. v3.6.0, 10K endorsements. Reqs FLM, KID, SPID, OAR, MCM Helper, SKSE — all baseline. "Living the world" homecoming layer.],
  [Discipline-first route],
  [Alternative],
  [all],
...
#table(
  ...
  [SKSE plugin — character wetness VFX (soak/dry gloss on skin, hair, armor, weapons) from swimming, rain, and snow. Roof/heat-source detection, NPC support, PBR-aware. 1.1K endorsements, v1.5.0a.],
  ...
  [Ships visible offshore in the Sea of Ghosts near Solitude, Raven Rock, and Tel Mithryn. Animated version. Fits the living-coast world-feel; animated ships and dock layer are in → `World Content` POI.],
)
...
#table(
  ...
  [OAR-based animations for campfire actions (add fuel, cook, eat at fire). Requires OAR, FormList Manipulator, MCM Helper. Campfire (Chesko) no longer hard requirement — evaluate against Starfrost-frame cooking.],
)
...
#table(
  ...
  [Add-on],
  [Card ("Battle of Heroes"), dice, and lottery mini-games at any tavern + tavern vouchers (3 free nights). v1.2.1, SKSE-free. Top-down camera (Witcher/Fable-like); Xbox One port exists. *Verify gamepad inputs in testing* (2016-era SkyUILib UI).],
  ...
  [Add-on],
  [Scales the price globals upward for a harder-sticker-shock economy (user-approved). Install alongside 55207; values remain configurable via the base mod's globals.],
)
...
#table(
  ...
  [Hand-authored new-cave/dungeon pack in the Velothi Mountains (Dunmer smuggling operation). V3.1, self-contained cells, vanilla-asset-only (PS4-origin), no scripts/quests, navmesh + LOD generated. *Level 15+ recommended* — a somewhat challenging explorer experience.],
...
#table(
  ...
  [Dark Souls-inspired island of Pharos — no quest, no hand-holding, level 15+ recommended. New enemies + an Ancient weapon set (Trinity of Chaos). LoTD patch available. 12,166 endorsements.],
)
...
#table(
  ...
  [*AI-voiced (ElevenLabs)* — accepted per AI gate decision (2026-08-07). AdamDunmer. Verify vs Riften-area mods (City of Crossed Daggers - Riften, JK's Riften Outskirts).],
  ...
  [*AI-voiced (11labs)* — accepted per AI gate decision (2026-08-07). AdamDunmer. The Great Town of Shor's Stone (town geometry) has no overlap expected.],
)
...
#table(
  ...
  [v2.2.1 (2020). Original v1 (#link("https://www.nexusmods.com/skyrimspecialedition/mods/20143")[mod 20143]) superseded — install Revamped only. Verify 1.6.1170.],
)
...
#table(
  ...
  [Add-on],
  [Unrelenting Force (player or dragons) instantly destroys weaker skeletons and pushes their bones apart. No vanilla record edits; configurable word level via ini. Requires PapyrusUtil + Po3 Extender + Po3 Tweaks + SPID (all baseline). Fits the locked Skeletons SE direction.],
...
#table(
  ...
  [Alternative],
  [#1, #2, #4],
  [50 hand-crafted weapons (craftable/lootable/unique finds) from the classic pack. Older era (Dec 2020) — check vs established weapon packs; modern alternatives (Billyro packs, Reforging - To the Masses) fill the same niche. LoTD display patch exists.],
...
#table(
  ...
  [Authentic Jeremy Soule compositions: 8 curated exploration tracks (Skyrim + Solstheim exploration/sunset/night categories). HD 192kbps, lossless-sourced, ESL option. Link fixed 2026-08-07 (was pointing at the unrelated texture mod 33903).],
...
Run → @main-character-voicing-custom-voice-pack-pipeline twice with two ElevenLabs voice IDs (Laura Bailey female, Graham McTavish male). Package as separate MO2 mods and enable one at a time.
