...
Landscape and rock PBR overrides (Faultier's PBR Landscapes, Enhanced Rocks and Mountains) are decided in → @graphics-terrain-terrain--flora. PGPatcher applies mesh patches across all separators — re-run once ALL PBR texture mods (Textures, Terrain, Characters, Lighting) are stable.
...
> *Terrain & rock PBR* is owned by → @graphics-terrain-terrain--flora: Faultier's PBR Landscapes 4k (125308) fills VPBR's terrain gap; Enhanced Rocks and Mountains (121336) is the locked mountain/rock baseline. Do NOT install `Faultier's Skyrim AIO` alongside VPBR — same texture paths.
...
4. Install landscape and rock PBR overrides per → @graphics-terrain-terrain--flora (Faultier's PBR Landscapes, ERM).
...
#table(
  ...
  [Removes dirty brown vertex-color shading on snow statics for a clean fresh-snow look. Meshes based on vanilla/SMIM/Assorted Mesh Fixes. FOMOD has optional patches for Simplicity of Snow, Snozz' Resource Pack, Enhanced Rocks and Mountains, Wyrmstooth, Deadly Dragon Lairs. Fits the CS clean-snow direction.],
)
