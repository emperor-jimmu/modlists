...
- **Better Dynamic Ash (54754)** is also double-pass per Faultier — keep for Solstheim worldspace edits only, with BDA's ESP loading **before** the PBR landscape ESP. BDA is standalone (52 KB plugin, no patcher required) — the Synthesis `BDSPatcher` was removed from Stage 6 because it only handles BDS v2 snow materials, not ash (source-verified, 2026-08-11).
