...
- **AutoSeasons** ([Nexus](https://www.nexusmods.com/skyrimspecialedition/mods/188381)) — **Adopted seasonal swap-data generator (2026-08-21).** Standalone tool scanning the load order for seasonal texture/mesh variants (recognizes `_WIN`/`_AUT`-style naming) and generating the SoS plugin + `Data/Seasons/*.ini` swap data, plus PBRNIFPatcher json rules for the downstream PGPatcher run and optional Terrain Helper parallax. Complements the locked Turn of the Seasons baseline (covers seasonal variants beyond TotS's set; skips records already covered by existing `Data/Seasons` inis). Pipeline: AutoSeasons → PGPatcher → DynDOLOD. **Verify generated output against the manual TotS setup in the seasonal LOD pass before relying on it for the 4× seasonal build.**
