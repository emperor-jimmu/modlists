...
## Mods for review — VERDICTS (2026-08-12, UI prerequisites)
...
### Adopted — prerequisite of Compass Navigation Overhaul

- **Infinity UI (74483)** — ADOPTED as Compass Navigation Overhaul prerequisite (user instruction). alexsylex's SKSE framework for loading SWF patches into menus/HUD elements; CNO's Nexus page lists it as a hard requirement (alongside Address Library + SKSE, both baseline). Also used by Local Map Upgrade (129756, baseline) and Dragon's Eye Minimap (135489). CoMAP 4.1.0+ DLL is built for Infinity UI 2.0.2+ / CNO 2.2.0+ compatibility — both versions in stack. `modlist-ui.md` → UI Framework Prerequisites.

## Mods for review — VERDICTS (2026-08-15, 6-mod batch)

Reviewed 6 submitted mods against Elder Wilds pillars (CS-compatibility, third-person/gamepad parity, redundancy, conflicts, overpower/cheats).

### Already in modlist (no new entries needed)

- **Lore-Friendly Load Screen Compendium (138294)** — Already locked UI baseline (`modlist-ui.md` → Loading Screens). 6,981 endo, v2.1, by ChildofDragons. Requirements (Improved Loading Screen Colors 61712 + SSE Display Tweaks 34705) already in stack. No change.
- **Sewer Rats - Mihail Monsters and Animals (137591)** — Already in `modlist-creatures.md` → Mihail Creature Additions. 1,340 endo, v1 (Dec 2024). No change.
- **Thrones Expanded (139544)** — Already in `modlist-graphics-textures.md` → Mesh Improvements - Furniture & Props. 1,673 endo, v1.2.1. Informational only: author discloses unfinished collisions (vanilla collision boxes, "100% functional"); optional BOS version requires Base Object Swapper (baseline). No change.

### Adopted — resolved from tentative

- **The Ratway Passages (138534)** — ADOPTED (2026-08-15, user instruction; resolved from tentative). YeahButIronically, 325 endo, v1.1.1 (updated Aug 2026 — active). New dungeon connecting the Ratway to Honeyside, Riften Jail/sewers, Hall of the Dead, Beggar's Row, and outside the city walls. Fits "new content" + "big dark world" pillars; local traversal only — no conflict with the locked Journeyman fast-travel stack. Well-received on r/skyrimmods ("why wasn't this in the base game"). `modlist-world-content.md` → Dungeons And Ruins baseline. **Build verification:** standard xEdit quick-conflict pass vs the Riften stack (The City of Crossed Daggers - Riften Expansion 168629, Caught Red Handed 65708 jail edits, Honeyside/player-home edits); confirm ESL flag + navmesh; sewer section is self-contained (Skyrim Sewers not in list); 1.6.1170 test launch per `modlist-performance-testing.md`.

### Tentative — evaluate before committing

- **Ashe - Crystal Heart (135085)** — Tentative (adoptable, verify gates). Martimius (SDA author), 8,876 endo, #1 Mod of the Month Dec 2024, #3 Most Popular 2024, v1.3.6 (May 2026). DLC-sized fully-voiced Breton follower: 3,000+ lines, personal quest, new lands, slow-burn romance (OStim or SFW fade-to-black). **No hard requirements**; every optional integration maps to a locked baseline: OAR (all animations OAR since v1.3, DAR fully removed), FSMP (physics), High Poly Head (head), SkyPatcher (bed-compat config explicitly supports in-list Rally's Noble Furniture / Rustic Repose / Sleipnir Beds), OStim Standalone (adult baseline), BFCO (FOMOD offers MCO/BFCO movesets — MCO rejected, BFCO locked), SDA (baseline) via Asherana banter patch (167123, 1,000+ lines). **Flags:** script overhead (r/wabbajack warns of constant background scripts in large lists — benchmark per `modlist-performance-testing.md` before locking), "main character syndrome" (edits existing NPCs/worldspaces — xEdit pass vs Solitude Winking Skeever cell + NPC/city ownership), tone (anime/Nier-2B-inspired visual direction vs grim-dark — weigh against the anbeegod FF-styled-follower precedent in `modlist-npcs.md` → Tone & Genre Fit), collection inclusion requires contacting the author per mod page. `modlist-npcs.md` → Custom Followers (tentative row added). **Choose:** BFCO moveset, OStim or SFW variant; install near bottom of load order; run FacegenBaseline; verify new-worldspace / DynDOLOD impact.

### Rejected

- **Unmoored - Skyrim Wayshrines (134945)** — REJECTED. Kheree, 298 endo, v2.1.0. ESO-style wayshrine network: 9 intact hold wayshrines with instantaneous teleport + 2 teleport spells (Phasing = random, Warp = any lit wayshrine) + random-teleport ruins + short voiced questline. **Direct conflict with the locked travel-pacing baseline** (`modlist-world-feel.md` → Travel Pacing And Carriage Fast Travel Rules): Journeyman (92220) gates fast travel behind the crafted/purchased Travel Pack, Fast Travel Cost (20200) prices map travel, Navigation Restrictions - NG (129086) adds survival travel pressure. Unmoored's free instantaneous teleport bypasses the entire gate — functionally a cheat against the list's intended travel friction. Secondary: 9+ new map markers (Atlas/CoMAP UI clutter) and worldspace cell edits near hold capitals. Well-made and community-liked (lightweight SWIFT alternative), but mechanically opposite to the travel design. Revisit only if the travel-pacing baseline is deliberately loosened.

## Mods for review — VERDICTS (2026-08-20, 7-mod batch)

Reviewed 7 submitted mods against Elder Wilds pillars (CS-compatibility, third-person/gamepad parity, locked-baseline conflicts, progression-dial fit, maintenance status). Sources: Nexus pages + r/skyrimmods community signal.

### Already in modlist (no new entry needed)

- **Biggie Traits (136384)** — Already Baseline in `modlist-expanded-character.md:77` (Character Traits And Identity Hooks). Review confirms: shazdeh2, 1,575 endo, v1.0.43 (Jul 2026 — active), ESL-flagged, MCM-configured. Community signal strong — LoreRim staple; r/skyrimmods treats it as the go-to for meaningful character-defining choices. Simonrim-aware (spell tagging for Apocalypse/Odin/Mysticism via optional Spell Classes). No hard dependencies (SkyUI for MCM is baseline). Minor gamepad note: trait selection is a power + menu (gamepad-fine); trait *removal* is done via Active Effects hover + R key (costs a dragon soul) — verify the gamepad button binding during testing. No change.

### Not adopted — removed after interim adoption (user instruction, 2026-08-20)

Progression stance corrected (2026-08-20): **normal progression mechanics, power fantasy leveled (eventually)** — leveling starts at a normal (vanilla) pace; the player's power ramps as content-fed XP and the static world stop scaling, so they eventually surpass almost everyone; level cap raised to **500** (from 250) to give the ramp room. Use-based stat growth and fatigue/regen-cap systems are not on-dial. The two interim adoptions below are rescinded; both section-file rows are reverted.

- **Exhaustion - Incremental Fatigue (94574)** — NOT ADOPTED (removed per user instruction). tjhm4, 1,043 endo, v2.1.0 (Sep 2025), ESL-flagged, zero performance impact. Initially adopted as a lightweight fatigue/injury layer (stat-depletion lowers regen caps; sleep restores; requires SkyUI + Scrambled Bugs, both baseline). Rescinded: the list wants "normal" progression without a fatigue/regen-cap system layered on top — Starfrost (97536, locked) already owns the fatigue axis as a needs stat. Row removed from `modlist-expanded-survival.md`; the Valhalla-exhaustion MCM note is moot.
- **Exercise - Incremental Growth (94663)** — NOT ADOPTED (removed per user instruction). tjhm4, 886 endo, v2.3.0 (Dec 2025). Initially adopted under a then-recorded "vanilla level-up dial is not a goal" reading; the corrected direction is power fantasy *through normal leveling* — use-based attribute growth (damage→health, casting→magicka, sprint/power attacks→stamina) is not on-dial. Chain **Skyrim Skill Uncapper for SE and AE (82558)** (Kasplat, v2.2.3) is chain-moot — it existed only to zero vanilla level-up gains for an exercise-only playthrough. Rows removed from `modlist-expanded-character.md`.

### Tentative — evaluate before committing (no section entry yet)

- **Additional Traits for Biggie Traits (149770)** — Tentative. LonlonSkyrim, 289 endo, v1.3.6.1 (Jul 2026 — active), ESL-flagged, requires Biggie Traits (baseline). 16 extra traits (Wayfarer, Paladin, Merchant, Thornmail, etc.); the non-Requiem main file is the candidate (Requiem file irrelevant — no Requiem in list). The author's "power level is rather high" caveat is **relaxed by the power-fantasy direction** (2026-08-20) — high-power traits are not disqualifying. Remaining gates: author describes himself as "still learning"; trait-by-trait quality/balance is unvetted; curation question of trait bloat over the curated base; verify Simonrim spell-cost/restoration interactions. Community treats the add-on as part of the accepted Biggie suite (used alongside Refitted variants in LoreRim/Nordic Souls). Evaluate: which traits merit adoption, and whether the curated base already covers the space.

### Rejected

- **Master of One - A Double-Edged Perk Overhaul (47024)** — REJECTED. tjhm4, 495 endo, v2.1.0 (last updated **Jul 2024** — stale). Full perk overhaul where every perk has benefits + drawbacks across 5 ranks; **incompatible with every other perk overhaul, including the locked Adamant baseline** (`modlist-expanded-magic.md:10`). Requires Attack Speed Framework + KID + Scrambled Bugs + SPID + SkyUI + SKSE (most already baseline). The double-edged-build axis is already owned at trait level by Biggie Traits (baseline) without replacing the perk tree. Revisit only if the Adamant perk baseline is deliberately reopened.
- **Vokriinator - Choice Cuts (26702)** — REJECTED. kb4 and his harem, 3,421 endo, v6.15.3 (last updated May 2025). Kitchen-sink mega-merge of Ordinator + Vokrii + Adamant + SPERG + Path of Sorcery that **replaces the entire locked Simonrim perk stack** with a merged mega-tree. Requires 8+ masters (Ordinator, Vokrii, Adamant, SPERG + Optimised Scripts for SPERG, Path of Sorcery, Ordinator Combat Styles, Dynamic Animation Casting NG, Po3 Papyrus Extender) and ships 15 files with an Odin/Pilgrim patch matrix — a heavy patch/maintenance surface that contradicts curation guardrails (clean Simonrim family, low patch burden). Noteworthy artifact (it does incorporate Adamant content), but mechanically and operationally the opposite of the curated Adamant + Mysticism baseline.
- **Take Notes - Journal of the Dragonborn SSE (13570)** — REJECTED (gamepad parity). ath3nos, 7,334 endo, v1.0a (last updated Jan 2024; author: source code lost, development slow). Beloved journal mod, but the journal is a **text-entry UI requiring a physical keyboard** — no on-screen keyboard; navigation is mouse-first; chapter customization needs the mouse; the author explicitly cannot confirm SSE controller support ("designed to work best with a mouse & keyboard"). Community consensus (r/skyrimmods, r/ultimateskyrim): gamepad users keep a keyboard beside them; no gamepad fix exists. Violates the hard third-person/gamepad parity directive (reading/journaling must work third-person with a gamepad; no keyboard/mouse requirement is acceptable). Also stale AE maintenance. **Removed the Alternative row from `modlist-expanded-character.md:15`.** Revisit only if a gamepad-usable journal (on-screen-keyboard text input) appears, or the parity rule is deliberately waived for this interaction.

### Section file changes

- `modlist-expanded-survival.md` → Survival Systems: Exhaustion row added then removed on reversal — **no net change** (file restored to original).
- `modlist-expanded-character.md`: Take Notes Alternative row removed (rejected on gamepad parity, stays removed); Exercise + Skyrim Skill Uncapper rows added then removed on reversal — **no net change** to Character Progression Mechanics.
- `guide/modlist-design-philosophy.md`: progression stance reconciled (normal leveling mechanics, power-fantasy leveled outcome; vanilla/slow-XP suggested-dial text corrected).
- `guide/modlist-expanded-character.md`: Experience configuration updated — `iMaxPlayerLevel` raised **250 → 500**; curve/rationale notes reconciled to the refined direction (start at a normal pace, ramp to surpass almost everyone).

## Mods for review — VERDICTS (2026-08-21, 6-mod TODO batch)

Reviewed the 6 submitted mods from `TODO.md` against Elder Wilds pillars (CS-compatibility, locked-baseline conflicts, toolchain fit, maintenance signal, third-person/gamepad). Sources: Nexus pages via the 2game.info mirror, the arca.live 툴리우스 fresh-mods digest (2026-08-21), r/skyrimmods, and the AutoSeasons GitHub repo (Cl3mus33/AutoSeasons — primary source for the toolchain).

### Tentative — toolchain monitor (no section entry yet)

Three Cl3mus toolchain utilities (**AutoSeasons**, **AutoBlend**, **BOSPriority**) — all released within the last 1–2 days (Aug 20–21, 2026); zero endorsements, GitHub 0 stars/0 forks/0 issues. Too new to lock into the toolchain; per the Lush Dragonsreach Pools precedent, re-evaluate once they establish community signal and pass verification against the locked pipeline. Grouped here because the author designs them as one pipeline: `AutoSeasons → AutoBlend → PGPatcher → DynDOLOD`.

- **AutoSeasons (188381)** — **PROMOTED to adopted (2026-08-21, user instruction).** Cl3mus, released 2026-08-21. Standalone tool scanning the load order for seasonal texture/mesh variants (recognizes `_WIN`/`_AUT`-style naming, not just SoS conventions) and generating the Seasons of Skyrim plugin + `Data/Seasons/*.ini` swap data; duplicates seasonal records, authors PBRNIFPatcher json rules for a downstream PGPatcher run, optionally adds landscape parallax via Terrain Helper integration, swaps/drops seasonal grass meshes. Built on PGPatcher's scanning core (hakasapl) + Mutagen + nifly; GPLv3; supports SE/GOG/VR/Enderal. **Directly serves the locked seasonal baseline** (SoS SKSE 62861 + Turn of the Seasons 63623, `modlist-world-feel.md:10`) and integrates two locked baselines (PGPatcher 120946, Terrain Helper 143149). Highest-potential tool of the trio — the seasonal LOD workflow is the list's most expensive operational piece (`modlist-graphics-lod.md` → Seasonal LODs). **Gates:** verify generated plugin/ini quality against the manual Turn of the Seasons setup; confirm PBR rules output against TomatoRim PBR Landscapes + PGPatcher; test in the seasonal verification pass before committing. Entry: `modlist-world-feel.md` → Timescale And Calendar Adjustments (seasonal swap-data generator row) + `modlist-graphics-lod.md` → Seasonal LODs cross-ref.
- **AutoBlend (188398)** — Cl3mus, v1.0.9. Landscape blending tool: scans landscape textures, generates mesh patches converting landscape statics from alpha-testing to alpha-blending, plus an output plugin with TextureSets/Alternate Texture overrides. Runs after AutoSeasons, before PGPatcher. **Gate: verify interplay with the locked PBR landscape baseline (TomatoRim 177621 + ERM 121336) and PGPatcher** — AutoBlend generates exactly the class of plugin/mesh output PGPatcher statically owns; only meaningful alongside AutoSeasons.
- **BOSPriority (188542)** — Cl3mus. Scans all BOS `_SWAP.ini` files, resolves conflicting keys with a priority picker, generates an AIO_SWAP.ini. BOS is a heavily-used baseline in the list (Regional Food Barrels, Various Immersive Coinbags, HFs series, etc.); a swap-conflict resolver is useful for a 400+ load order, but BOS conflicts are normally deterministic last-write-wins — value is consolidation, not correctness. Monitor.

### Tentative — evaluate before committing (no section entry yet)

- **Miscellaneous Plus Plus (188168)** — **PROMOTED to adopted (2026-08-21, user instruction).** Flowrifto, Aug 2026. Quest-logic overhaul of "A Return to Your Roots" (Crimson Nirnroot): makes the Sarethi quest-giver killable, removes the quest-item flag from Sinderion's Field Journal, refines nirnroot counting, gates progression on reading the journal. Same author as **A Chance Arrangement Plus Plus (175885, adopted at `modlist-world-content.md:353`)** — the Plus Plus series has precedent in the list. Small contained surface; author flags possible conflicts with mods touching the same quest/NPCs/journals. **Gates:** verify USSEP (baseline) record overlap on the crimson nirnroot quest + killable-quest-giver quest-flow behavior before adopting. Entry: `modlist-world-content.md` → Quest Expansions (Vanilla+) row.

### Rejected

- **Promises To Keep Plus Plus (188756)** — REJECTED. Flowrifto, Aug 19 2026. Expands MS03 ("Promises to Keep" — Sibbi/Louis Letrush/Frost): relaxed progression order, new branches incl. reporting to Maven Black-Briar, possible deaths of Louis Letrush and Sibbi, edited vanilla voice lines, reduced quest-item flags. **Hard conflict with the locked The Choice is Yours (3850) baseline** (`modlist-expanded-followers.md:11`): both overwrite `QF_MS03_0001DA3C.pex`; the author explicitly lists TCIY as incompatible and no patch exists. Same pattern as Missing Follower Dialogue Edit (rejected for dropping the RDO bridge) — a quest layer that breaks a locked baseline is incoherent.

### Not adopted (chain-moot)

- **Hammet Dungeon Pack III - Unique Rewards (189005)** — NOT ADOPTED (chain). Xtudo, Aug 2026. Visual-only replacer of HDP3 dungeon rewards with 14 unique items (stats/temper/crafting conditions preserved), ESL-flagged, safe mid-game. Requires **Hammet Dungeon Pack III SE (188925, released Aug 20 2026)** — not in the list (only Hammet's Dungeon Pack 1 SE 12186 is baseline, `modlist-world-content.md:184`) — plus Daedric Armor SE and Northern God Armor SE textures, neither present. Same pattern as Interesting NPCs Visual Overhaul (rejected chain: a replacer for a non-adopted base is moot). Revisit only if HDP3 is adopted — Pack III itself is a plausible future dungeon-pack candidate given Pack 1 is baseline and the series is well-regarded on r/skyrimmods.
