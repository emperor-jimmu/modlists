...
**Status Ladder:**
- `🔴 OPEN` — Not yet decided. Research complete, pros/cons documented.
- `🟡 TRIAGE` — Being evaluated. Testing or additional research in progress.
- `🟢 DECIDED` — Path chosen. Recommendation accepted but not yet integrated into guide files.
- `🔵 LOCKED` — Chosen. Mod picked, baseline documented in the relevant `modlist-*.md` section file.
- `⚪ DEFERRED` — Postponed. Will revisit after higher-priority decisions are locked.
- `⛔ REJECTED` — Considered and ruled out. Will not be adopted.
