...
**MO2 Separator:** `05 Expanded Systems` → `05d Crafting & Economy`

Part of the [`Expanded Systems`](modlist-06.md) section. See also: `06a` (Character & Progression), `06b` (Magic & Perks), `06c` (Survival & Needs), `06e` (Followers & Reputation).

## Alchemy Overhaul → `05d`
...
## Smithing And Crafting Expansion → `05d`
...
## Artifact And Unique Item Overhauls → `05d`
...
- Stacking `Reliquary of Myth` with `ArteFakes` and `Unique Items Tweaks` touches every unique-item record in the game. They are designed to stack, but verify the `xEdit` patcher chain in `modlist-14d.md` does not re-balance the same records (the Synthesis `Weapon Stat Synthesis Patcher` should be configured to skip unique items if these mods are present). → `05d`
...
- The artifact stack does not conflict with the `Synthesis` patcher config in `modlist-14d.md`. → `05d`
...
- Keep `Immersive Weapons` as the legacy comparison route if the list wants maximum raw weapon count (230+ vs 150+) and can tolerate an unmaintained mod approaching a decade old. It pairs with the synthesis patcher chain in `modlist-14d.md` (the `OWLLeveledListAddition` and `SpeedandReachFixes` patchers are designed with it in mind). Prefer Heavy Armory unless the list specifically needs IW-exclusive weapon variants. → `05d`
...
- `Heavy Armory` uses standard leveled-list integration and does not require synthesis patcher intervention the way `Immersive Weapons` does. If using `Immersive Weapons` instead, the synthesis patcher chain in `modlist-14d.md` has explicit `Bash Tags` and patcher entries for it — do not skip `OWLLeveledListAddition` and `SpeedandReachFixes` if IW is present. → `05d`
...
- All three weapon mods add new models, which means the `Pandora` and `BodySlide` rebuild triggers in `modlist-14d.md` fire whenever any of them is updated. → `05d`
...
- The chosen weapon stack does not conflict with the synthesis patcher entries in `modlist-14d.md`. → `05d`
...
- Use `Immersive Armors` as the content-variety baseline. It adds 200+ new armors (light, heavy, mage, fur, leather) with leveled-list integration, all designed to fit the vanilla art direction. It is the canonical armor content pack (206K endorsements, but last updated November 2016 — functionally stable as a leveled-list injector, though approaching a decade unmaintained; verify game version compatibility before locking) and pairs with `Immersive Weapons` for parallel content addition. The synthesis patcher chain in `modlist-14d.md` already has explicit `Bash Tags` and patcher entries for it. → `05d`
...
- `Immersive Armors` is a large mod and the synthesis patcher chain in `modlist-14d.md` already has explicit `Bash Tags` and patcher entries for it. Do not skip the `OWLLeveledListAddition` or `Weapon Stat Synthesis Patcher` if `Immersive Armors` is present.
...
- All IA companion mods add new models, which means the `Pandora` and `BodySlide` rebuild triggers in `modlist-14d.md` fire whenever any of them is updated. After any IA mesh update, run BodySlide and rebuild all IA batches, then regenerate the retexture outputs. → `05d`
...
- The chosen armor stack does not conflict with the synthesis patcher entries in `modlist-14d.md`. → `05d`
- New armors feel like meaningful finds in dungeon and bandit loot, not leveled-list padding.

## Clothing And Wardrobe Extensions → `05d`
