...
| [Accessories](https://modrinth.com/mod/accessories) 1.1.0-beta.53+1.21.1 | Data-driven accessory slot API (Curios-style) — covers Artifacts' native slots; runs alongside Curios |
...
- **ShatterLib (OctoLib)** is required by Relics — Octo-Studios library, new dependency (`OctoLib-NEOFORGE-0.6.2+1.21.jar`, auto-installs via XMCL from the Relics page). Architectury API (already in pack) covers OctoLib's framework expectation.
- **Curios API** provides the accessory slot system used by Relics and Immersive Armors. Adds an equipment GUI (keybind `Y`) with ring, amulet, belt, gloves, back, and charm slots. Auto-installed dependency — no separate config needed. Runs alongside Accessories (below); Artifacts equips via either backend.
...
**Dependencies**: Curios (already in pack) + Accessories (below) cover both optional slot backends — equip Artifacts via either menu.

### Accessories — Second Slot Backend

**Version pin (Sep 2026)**: Accessories **1.1.0-beta.53+1.21.1** (`accessories-neoforge-1.1.0-beta.53+1.21.1.jar`) — Wisp Forest, NeoForge 1.21.1, MIT. Data-driven accessory slot API in the Curios/Trinkets lineage; Artifacts lists it as a native optional backend alongside Curios, so Artifacts trinkets slot in with zero shims. Runs alongside Curios (Relics + Immersive Armors stay on Curios) — no migration, no conflict.

**Dependencies**: oωo (owo-lib) 0.12.15.5-beta.1+1.21 (required, auto-installs via XMCL from the Accessories page). Note: the 1.21.1 line is beta-channel — latest available, no stable exists.
