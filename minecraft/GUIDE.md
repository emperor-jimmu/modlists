...
6. Armored Legacy — author requires top placement; needs EMF + ETF mods
7. Mod Descriptions — Field Guide companion, lang-only; any position works
8. Fresh Animations
9. Fresh Animations: Objects (directly above FA — author requirement)
10. Fresh Animations: Emissive (directly above FA — author requirement)
11. Enhanced Boss Bars
12. Visual Traveler's Titles (title-card overlay — order-flexible, grouped with UI tier)
13. RAY's 3D Rails (3D rails)
14. RAY's 3D Ladders (3D ladders — pair with 3D Rails; disjoint assets)
15. Better Lanterns
16. (Bee's) Fancy Crops
17. Rekindled CTM (Fire Rekindled addon — load above it)
18. Fire Rekindled
19. Motsch's Better Leaves (pair with Cull Leaves mod)
20. Bushy Pink Petals, Wildflowers & Leaf Litter
21. Modded Omelet (spawn egg items)
22. Mace Fusion (mace item model)
23. Fresh Waystones Texture (Waystones block + item)
24. Overlay's — needs Continuity + Sinytra Connector mods
25. MissingSoundsFix — no visual impact, order irrelevant
26. Create Immersive Aircrafts (IA vehicles + items; above the base pack)
27. Faithful 32x — base layer (overlays above win)
...
| [Dark Loading Screen Neoforge](https://modrinth.com/mod/dark-loading-screen-neoforge) 1.0.1                                              | Dark loading screen (NeoForge-native port) — customizable colors/bar/logo via `config/dark-loading-screen.json`                             |
...
- **Dark Loading Screen Neoforge** — NeoForge-native dark loading screen (1.0.1, 1.21.1 ✅ via Modrinth API, `dependencies: []`, MIT, client-only). Replaces the pinned CurseForge [DLS] file — no more deep-file pin. Customizable (background/bar/border/logo colors, fade times) via `config/dark-loading-screen.json`; Cloth Config (in pack) enables the in-game color screen.
