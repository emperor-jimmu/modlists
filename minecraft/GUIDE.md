...
| [Fancy Notify](https://www.curseforge.com/minecraft/mc-mods/fancy-notify) 1.0.0         | Overhauls all vanilla toasts — recipes, advancements, systems, music, biomes, weather, logins, screenshots — with per-type filters; replaced AP + FT (Aug 2026)                                                                                                                   |
...
- **Better Advancements** — overhauled advancements UI, open with `L`. Pairs with Fancy Notify.
- **Fancy Notify** — overhauls ALL vanilla toasts with unique sprites: recipes, advancements, systems, music (now-playing track), biomes, weather, player logins, screenshots. Per-type **Filters** screen (Mods screen → Fancy Notify) — set **Biomes OFF** (Traveler's Titles owns biome titles). Also: transparency, width, anchor, List/Line orientation, animation, max amount, UI particles. Replaced Advancement Plaques + Fancy Toasts (Aug 2026; CF-only, zero deps; Iceberg stays for Item Borders).
...
**Game Rules → Chat → disable all options** (Command Feedback, Command Block Output, Reduced Debug Info, Show Death Messages, Show Recipe Messages, Announce Advancements). Keeps the chat clean — progression popups still appear via Fancy Notify.
