...
| [C2ME](https://modrinth.com/mod/c2me-neoforge) 0.4.0-alpha.0.120+1.21.1     | Parallelizes chunk generation/loading/IO across CPU cores — the biggest worldgen speedup in the pack. Server-side; pairs with Fast Noise + Lithium. ⚠ alpha backport devbuild (flagged)                            |
| [Fast Noise](https://modrinth.com/mod/zfastnoise) 1.0.13+1.21.1+neoforge          | Vanilla-parity worldgen optimization — faster chunk gen (10–18% overworld, much more in Nether/End). Replaces NoisiumForked (archived — its successor)                                                       |
...
**Fast Noise** — replaces NoisiumForked (adopted Jul 2026; NoisiumForked archived May 2026, its own maintainer recommends Fast Noise). Vanilla-parity worldgen optimization: only the block/biome storage paths in `NoiseChunkGenerator` are swapped, so terrain output, datapacks, and modded worldgen (Terralith/Tectonic) are unchanged. Zero deps. Risky mod-compat-affecting optimizations disabled by default. [VERIFY] at launch: loads cleanly with Lithium (load-order fix in the mod's changelog) and the Terralith/Tectonic stack generates correctly.
**C2ME (NeoForge)** — parallelizes chunk generation, loading, and IO across CPU cores (the single biggest worldgen speedup available). Server-side — runs in the single-player integrated server. Zero deps; the standard pairing with Fast Noise (block-storage opt) + Lithium (game logic) — Fast Noise's incompatible list excludes C2ME, and C2ME's own benchmarks run zfastnoise + Terralith. ⚠ **Alpha backport devbuild** for 1.21.1 (`0.4.0-alpha.0.120`, auto-uploaded GitHub Actions; "alpha" is the project's standing versioning convention — Fabric original: 35M DL). [VERIFY] at launch: clean load with Lithium/Fast Noise; Terralith/Tectonic/Incendium terrain correct; no new world needed (parity).
...
| [Fresh Animations: Objects](https://modrinth.com/resourcepack/fresh-animations-objects) v2.1.2                                            | Official FA extension — animates chests, boats, minecarts, shulker boxes (wobble, slide, open/close). Load above Fresh Animations |
| [Fresh Animations: Emissive](https://modrinth.com/resourcepack/fresh-animations-emissive) v1.6.0                                          | Official FA extension — emissive textures for Drowned/Stray/Bogged, Iron Golem, Guardians, firing Ghast, angry Bee/Wolf, Zombie Villager, Piglins, Hoglins, Bat. Pairs with ETF + LambDynamicLights. Load above Fresh Animations |
...
| [Visual Traveler's Titles](https://www.curseforge.com/minecraft/texture-packs/visual-travelers-titles)                                        | Handmade dimension logos for Traveler's Titles entry cards — Overworld/Nether/End + Aether, Twilight Forest, Northstar planets. Uncovered dims fall back to text |
| [Dark Coffee GUI](https://modrinth.com/resourcepack/dark-coffe-gui) v2.2.1                                          | Warm dark coffee-themed 16x GUI retexture — all inventories, hotbar, hearts/armor/food icons, buttons, toasts. **Replaces Mindful Darkness' GUI darkening (full swap, Aug 2026)** |
| [Modded Coffee GUI](https://modrinth.com/resourcepack/modded-coffee-gui) v1.15.2                                    | Dark Coffee GUI's official mod-compat addon (same author) — coffee-styled screens for EMI, Create, Farmer's Delight, Waystones, Relics, Curios, Supplementaries, etc.; load above the base pack |
...
| [MissingSoundsFix](https://modrinth.com/resourcepack/missing-sounds-fix)                                                                       | Suppresses vanilla empty-sound warnings (salmon.ambient, cod.ambient, etc.) — fixes log spam from MC-97521       |
...
| [Create Immersive Aircrafts](https://www.curseforge.com/minecraft/texture-packs/create-immersive-aircrafts-resource-pack) 3.0                | Create-styled textures for Immersive Aircraft items & vehicles — pairs with its Recipes data pack                |
...
- **Fresh Animations: Objects** — official FA extension (FreshLX, v2.1.2 Jun 2026, 1.21.1 ✅ via Modrinth API, `dependencies: []`): animates non-mob entities base FA doesn't touch — chests (open/close anims, custom trapped-chest button), boats (wobble, slide on turns, land anims, idle oar positions), minecarts (wheels + track wobble), shulker boxes, ender chests. **Load directly above Fresh Animations**. 16x-vanilla-styled like the base — inherits the 16x-on-32x tradeoff already accepted for FA.
- **Fresh Animations: Emissive** — official FA extension (FreshLX, v1.6.0 Apr 2026, 1.21.1 ✅, `dependencies: []`): emissive textures for Drowned, Stray, Bogged, Iron Golem, Guardian & Elder Guardian, firing Ghast, angry Bee, angry Wolf, Zombie Villager, Piglins, Hoglins, Bat. Rendered via ETF (already in pack) and *lit* via LambDynamicLights — glowing mobs that cast real light (a firing Ghast lighting the cave is a genuine gameplay read). **Load directly above Fresh Animations**.
- **Enhanced Boss Bars** — themed boss bar designs for vanilla + Twilight Forest. For Cataclysm/Aether bars (custom rendering), also install the companion [Enhanced Boss Bars mod](https://modrinth.com/mod/enhanced-boss-bars-mod). **On install, set Jade's boss-bar options** (Main Menu ➜ Mods ➜ Jade ➜ Config ➜ Mod Settings):
...
- **Visual Traveler's Titles** — handmade dimension logos for the Traveler's Titles mod's entry cards (un_roman, 12.6M DL, co-authored with YUNGNIKYOUNG). Covers Overworld/Nether/End plus **Aether, Twilight Forest, and Northstar planets** (Mars/Mercury/Moon/Venus) — the pack's dimension lineup. Uncovered dims (Bumblezone, Deeper and Darker, Cataclysm arena) fall back to text titles, gracefully. No conflicts — title-card overlay touches nothing Dark Coffee/Faithful/CEM packs own. Standard Visual variant (Legacy's extra compat unneeded for our dims).
...
- **Modded Coffee GUI** — Dark Coffee GUI's official mod-compat addon (same author): coffee-styled screens for **EMI, Create, Farmer's Delight, Waystones, Relics, Curios, Supplementaries, AppleSkin, Overflowing Bars, Spell Engine, Immersive Aircraft**, Sophisticated Storage/Backpacks (partial — scrollbar/search stay grey) and more. **Not covered — stay bright vanilla**: Mekanism GUIs (blocks/items now 32x via Upscaled Mekanism 32x), AE2, MineColonies, dimension mods (Mindful Darkness darkened every GUI uniformly; that coverage is the loss). **Load directly above Dark Coffee GUI.** Main test versions per author: 1.20.1 / 1.21.1.
...
- **Create Immersive Aircrafts** — 16x retexture of Immersive Aircraft's crafting items, upgrade items, and vehicles in Create style (also renames some items in US English). Only touches IA assets — any position above the base pack works. Pairs with its Recipes data pack (Wave 2 Mobility / `datapacks/README.md`): textures and recipes are designed to match.
...
1. Dark Coffee GUI — GUI theme (16x coffee); wins the GUI surface pack-wide (vanilla + covered-mod screens; see Texture Packs). Above Improved Create/AE2: those 32x packs only overlap the GUI portions — item/block textures unaffected; move below them if you prefer the 32x mod GUIs
2. Modded Coffee GUI — Dark Coffee GUI's mod-compat addon; load directly above it

3. Improved Create 32x — author requires top placement; only overlaps the base pack
4. Improved Applied Energistics 2 — author requires top placement; only overlaps the base pack
5. Upscaled Mekanism 32x — Faithful-style Mekanism coverage; 32x tier, only overlaps the base pack
6. Mod Descriptions — Field Guide companion, lang-only; any position works
7. Fresh Animations
8. Fresh Animations: Objects (directly above FA — author requirement)
9. Fresh Animations: Emissive (directly above FA — author requirement)
10. Enhanced Boss Bars
11. Visual Traveler's Titles (title-card overlay — order-flexible, grouped with UI tier)
12. RAY's 3D Rails (3D rails)
13. RAY's 3D Ladders (3D ladders — pair with 3D Rails; disjoint assets)
14. Better Lanterns
15. (Bee's) Fancy Crops
16. Rekindled CTM (Fire Rekindled addon — load above it)
17. Fire Rekindled
18. Motsch's Better Leaves (pair with Cull Leaves mod)
19. Bushy Pink Petals, Wildflowers & Leaf Litter
20. Modded Omelet (spawn egg items)
21. Mace Fusion (mace item model)
22. Fresh Waystones Texture (Waystones block + item)
23. Overlay's — needs Continuity + Sinytra Connector mods
24. MissingSoundsFix — no visual impact, order irrelevant
25. Create Immersive Aircrafts (IA vehicles + items; above the base pack)
26. Faithful 32x — base layer (overlays above win)
...
| Mod                                                                                                               | Role                                                                                                                                                                  |
|-------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [LambDynamicLights](https://www.curseforge.com/minecraft/mc-mods/lambdynamiclights) 4.8.11+1.21.1                  | Dynamic lighting — pin 4.8.11 (Aug 2026 EOL bugfix line; fixes duplicate-keybind `options.txt` corruption). Client-only, zero new deps |
...
| [AttributeFix](https://www.curseforge.com/minecraft/mc-mods/attributefix) 21.1.3                                                            | Lifts vanilla attribute caps (armor 30+, health) so Overflowing Bars displays real values — requires Prickle + Bookshelf (both auto-installed)                         |
...
| [[EMF] Entity Model Features](https://modrinth.com/mod/entity-model-features)                                     | OptiFine-format custom entity models — powers Fresh Animations; requires ETF                                                |
...
| [Grassier Grass](https://modrinth.com/mod/grassier-grass) 1.4.5                                                  | Animated 3D grass blades with wind shader — lush swaying fields; Iris/DH compatible                                                                                                             |
...
| [Fancy Toasts](https://modrinth.com/mod/fancy-toasts) 1.5.0                                    | Modern advancement-popup presentation — clean unlock toasts; pairs with Fancy Notify (advancement popups)                                                                  |
| [Fancy Notify](https://www.curseforge.com/minecraft/mc-mods/fancy-notify) 1.0.1+               | Custom notification popups — recipe/system/music/screenshot/biome toasts with per-type filters; owns toast silencing (replaces Toast Control)                                |
...
| [JustEnoughBreeding](https://www.curseforge.com/minecraft/mc-mods/justenoughbreeding) 3.2.1                                     | EMI breeding lookup — food, egg output, tamed-flag for vanilla + modded mobs; client-only, zero required deps                                |
...
**Tooltip surface (Aug 2026 swap)**: **Obscure Tooltips owns tooltip rendering** — Modern UI's tooltip feature is **disabled via the Action Center** (`Ctrl+K`) so the two don't fight (both replace the vanilla tooltip renderer; Modern UI's docs acknowledge the conflict). Don't re-enable Modern UI tooltips while Obscure Tooltips is installed. **Legendary Tooltips** (ver 1.16 ~ 1.21.4) would fight Obscure Tooltips the same way — if it's ever added, configure it lower or remove it. **GUI dark mode moved to a resource pack** (Aug 2026 full swap): Mindful Darkness removed → **Dark Coffee GUI** + **Modded Coffee GUI** (see Texture Packs). Modern UI compatible: its documented conflicts are tooltip/text-engine mods, not GUI-texture resource packs (the instance already proves the surface with Faithful 32x); no shader-pipeline fight (unlike the old Dark Mode Everywhere watch). The pack's text recolor follows JSON font definitions (Modern UI honors them); NOSHADERS variant is the fallback if text looks off under Iris.
...
- **Overflowing Bars** — health/armor/toughness bars expand past vanilla limits — useful for high-HP builds from Relics/Artifacts bonuses and enchanted gear. Requires **AttributeFix** (next bullet) for values beyond vanilla caps — without it the bar renders numbers the game silently clamps.
- **AttributeFix** — lifts vanilla's arbitrary attribute caps (Darkhax, 21.1.3 NeoForge 1.21.1 ✅ via Modrinth API, `dependencies: [Prickle, Bookshelf]` both auto-installed, LGPL). Changes no stats by itself — only raises the ceiling so Overflowing Bars + Detail Armor Bar display real Phase 3 values. Zero config.
...
- **Dark Coffee GUI + Modded Coffee GUI** — dark coffee-themed GUI retexture (replaces Mindful Darkness, Aug 2026 full swap). Dark Coffee GUI restyles all vanilla inventories + HUD icons (hearts, armor, food, hotbar) in a warm coffee palette; Modded Coffee GUI (load directly above it) extends the style to EMI, Create, Farmer's Delight, Waystones, Relics, Curios, Supplementaries, AppleSkin, Overflowing Bars, Spell Engine, Immersive Aircraft, Sophisticated Storage/Backpacks (partial — scrollbar/search stay grey). **Not covered — stay bright vanilla**: Mekanism, AE2, MineColonies, dimension mods (Mindful Darkness darkened every GUI; that coverage is the loss). Modern UI compatible — no shader-pipeline fight; tooltip surface still Obscure Tooltips (Modern UI tooltips off). **Load in the UI tier directly above Dark Coffee GUI**.
...
- **JourneyMap + Integration** — minimap with entity dots, fullscreen map (`J`), waypoints (`B` to set), entity radar. Waystones appear as icons via the Integration mod. `J` → right-click sets a temporary waypoint. Compass to Map auto-pins Explorer's Compass finds.
...
- **ClientSort** — sorting with 4 modes (creative order, quantity, alphabetical, item ID) plus Fill Stacks, Transfer Matching, and a Button Editor. The pack's sort tool.
...
- **Better Advancements** — overhauled advancements UI, open with `L`. Pairs with Fancy Toasts for popup presentation.
- **Fancy Toasts** — modern advancement-popup presentation (clean unlock toasts). Replaces Advancement Plaques (Aug 2026). No config; zero deps.
- **Fancy Notify** — customizable notification popups: recipes, systems, music, screenshots, weather, biomes, advancements, with per-type filters (Mods screen → Fancy Notify → Filters). Recipe-toast silencing lives here (Filters → Recipes) — replaces Toast Control (Aug 2026). **Biomes filter OFF** (Traveler's Titles owns biome entries); music toasts complement The Immersive Music Mod. Re-adopted after the 1.0.0 startup crash (MusicManager NPE) was fixed in 1.0.1 — [Bivrik/FancyNotify#3](https://github.com/Bivrik/FancyNotify/issues/3) closed Aug 26 2026; ships explicit Fancy Toasts compatibility (FT owns advancement popups). CurseForge-only.
...
- **MEED** — curated descriptions + effect providers (which items/mobs/blocks/fluids apply each effect) for 600+ effects across 100+ mods. Addon for JEED/EMIffect. Covers pack effect-heavy mods: Aether, Deep Aether, Cataclysm, Relics, Artifacts, Simply Swords, Too Many Bows, Deeper and Darker.
- **JustEnoughBreeding** — EMI breeding lookup (food, egg output, tamed-flag) for vanilla + modded mobs. Client-only 3.2.1, zero required deps. Chill-living/farming fit; Jade owns growth info (different surface, no overlap).
...
| Mod                                                                                 | Role                                                                                                                                                    |
|-------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Mekanism: Weapons](https://www.curseforge.com/minecraft/mc-mods/mekanism-weapons)  | Phase 3 energy weapons — Meka-Tana (katana + teleport), Meka-Bow (energy arrows), Meka-Gun (laser). Polonium/atomic-alloy gated. Better Combat built-in |
...
Five mods work together for a complete train network:
...
Together: tracks → stations → navigation → interiors → detailing, with no overlap.
...
Phase 3 energy-weapon line sharing the "Meka" aesthetic: **Meka-Tana** (katana — high damage, teleport + sweeping modules), **Meka-Bow** (energy arrows, auto-fire, draw speed, gravity dampener), **Meka-Gun** (laser rifle with heat/cooldown system), plus exclusive modules (Weapon Attack Amplification — up to 4× damage, configurable; Looting V) and a Wireless Charger (block + optional Curios back-slot display). Requires only Mekanism (pinned 10.7.17.83), MIT license.
...
| Power bridge       | Crafts & Additions    | Kinetic ↔ FE                                          |
...
These schematics complement the Driftwood progression. All are free downloads — 16 from Abfielder.com (one of the largest schematic libraries, 700+ schematics, all free) and 5 from PhantomMarket. Each schematic is picked to serve a specific purpose in your journey from Cozy Roots through Industrial Ascent to Perpetual Ascent; the block count (non-air blocks) and volume (bounding box X×Y×Z) show the material commitment before you start.

| #  | Schematic                                                      | Source · ID                                    | Phase | Purpose                                                        | Style                    | Blocks (Volume)   | Difficulty   |
|----|----------------------------------------------------------------|------------------------------------------------|-------|----------------------------------------------------------------|--------------------------|-------------------|--------------|
| 1  | [Stone Medieval Tiny House](https://abfielder.com/Products/ProductDetails.php?id=13580) | Abfielder · 13580                              | 1     | Compact medieval starter with shop front                       | Stone & timber medieval  | 968 (3,136)       | Starter      |
| 2  | [Forest House](https://abfielder.com/Products/ProductDetails.php?id=2865) | Abfielder · 2865                               | 1–2   | Woodland home with room to grow                                | Rustic forest lodge      | 4,568 (30,912)    | Moderate     |
| 3  | [Ivy's Starter House](https://abfielder.com/Products/ProductDetails.php?id=2338) | Abfielder · 2338                               | 1     | Minimal first shelter — place in the first hour                | Cozy cottage             | 651 (1,980)       | Starter      |
| 4  | [Simple Medieval House](https://abfielder.com/Products/ProductDetails.php?id=7986) | Abfielder · 7986                               | 1–2   | 3-floor home with animal pen and full interior                 | Timber-framed medieval   | 3,654 (24,800)    | Moderate     |
| 5  | [Ultimate Survival Base](https://abfielder.com/Products/ProductDetails.php?id=2665) | Abfielder · 2665                               | 1     | Practical all-in-one survival base, easy-to-gather resources   | Rustic survival house    | 2,992 (10,200)    | Moderate     |
| 6  | [Trading Hall with Basement](https://abfielder.com/Products/ProductDetails.php?id=3396) | Abfielder · 3396                               | 2     | Villager trading hall — pairs with Guard Villagers and the colony economy | Grand stone hall         | 37,720 (176,900)  | Large        |
| 7  | [Large Farmhouse](https://abfielder.com/Products/ProductDetails.php?id=2539) | Abfielder · 2539                               | 1–2   | Farmstead base with room for crops and animals                 | Medieval farmhouse       | 2,871 (11,424)    | Moderate     |
| 8  | [Survival Friendly Cherry House!](https://abfielder.com/Products/ProductDetails.php?id=13576) | Abfielder · 13576                              | 1–2   | Cherry-blossom shell — no interior, furnish it yourself        | Cherry-blossom cottage   | 2,614 (14,280)    | Moderate     |
| 9  | [Medieval House – Fully Decorated Interior](https://abfielder.com/Products/ProductDetails.php?id=13041) | Abfielder · 13041                              | 1–2   | Survival-friendly decorated home                               | Stone & timber medieval  | 3,544 (12,474)    | Moderate     |
| 10 | [Oasis Sanctuary](https://abfielder.com/Products/ProductDetails.php?id=13573) | Abfielder · 13573                              | 2–3   | Landmark desert settlement with towers and gardens — a long-term project | Sandstone desert oasis   | 16,634 (227,766)  | Large        |
| 11 | [Minecraft Wooden Mansion](https://abfielder.com/Products/ProductDetails.php?id=12525) | Abfielder · 12525                              | 2     | Large mansion — Phase 2 upgrade from starter home              | Grand timber estate      | 16,992 (146,080)  | Large        |
| 12 | [Medieval House – Fully Decorated Interior](https://abfielder.com/Products/ProductDetails.php?id=12842) | Abfielder · 12842                              | 1–2   | Tall decorated fantasy-style home                              | Stone & timber medieval  | 4,238 (21,546)    | Moderate     |
| 13 | [Cozy Mountain Chalet](https://abfielder.com/Products/ProductDetails.php?id=13061) | Abfielder · 13061                              | 2     | Storage room, kitchen, brewing room and library in an alpine shell | Snowy mountain chalet    | 18,353 (66,048)   | Large        |
| 14 | [Medieval House – Fully Decorated Interior](https://abfielder.com/Products/ProductDetails.php?id=12891) | Abfielder · 12891                              | 1–2   | Compact decorated medieval home                                | Stone & timber medieval  | 4,855 (9,614)     | Moderate     |
| 15 | [ModernHouse](https://abfielder.com/Products/ProductDetails.php?id=4201) | Abfielder · 4201                               | 1–2   | Easy-build modern home                                         | Modern minimalist        | 5,956 (13,041)    | Moderate     |
| 16 | [Modern Mansion](https://abfielder.com/Products/ProductDetails.php?id=9485) | Abfielder · 9485                               | 2     | Medium-sized modern mansion                                    | Modern glass & concrete  | 14,074 (67,375)   | Large        |
| 17 | [Simple modern house!](https://market.phantom-node.com/post/simple-modern-h-fe5bfqfp) | PhantomMarket · simple-modern-h-fe5bfqfp       | 1     | Modern starter base                                            | Modern minimalist        | 1,523 (6,188)     | Starter      |
| 18 | [Solo Survival House!](https://market.phantom-node.com/post/solo-survival-h-a7lmgelv) | PhantomMarket · solo-survival-h-a7lmgelv       | 1     | Compact solo survival starter                                  | Modern compact           | 1,250 (10,472)    | Starter      |
| 19 | [Luxury Waterfront Villa](https://market.phantom-node.com/post/luxury-waterfro-0dd5wxfh) | PhantomMarket · luxury-waterfro-0dd5wxfh       | 2–3   | Waterfront villa with garage and private pool                  | Modern coastal villa     | 190,065 (379,600) | Mega         |
| 20 | [Modern Stone and Wood House](https://market.phantom-node.com/post/casa-moderna-de-qdkqmx4d) | PhantomMarket · casa-moderna-de-qdkqmx4d       | 1–2   | Riverside home with terraces and balconies                     | Modern stone & wood      | 5,126 (18,240)    | Moderate     |
| 21 | [Modern Big House](https://market.phantom-node.com/post/casa-moderna-y-jjda2sk1) | PhantomMarket · casa-moderna-y-jjda2sk1        | 2     | Large modern home                                              | Modern contemporary      | 61,877 (412,528)  | Mega         |

**Download**: For Abfielder schematics, visit `https://abfielder.com/Products/ProductDetails.php?id=<ID>` and click "Download" for the `.litematic` file. For PhantomMarket builds, open the post page and click Download — files ship as `.litematic` or `.schem`, both of which Forgematica loads. Place all files in your instance's `schematics/` folder; Forgematica picks them up on next launch.

**Difficulty tiers** (by block count): **Starter** < 2,000 — a weekend project · **Moderate** < 10,000 — a solid base · **Large** < 50,000 — a multi-session build · **Mega** ≥ 50,000 — a long-term megabuild

**Finding more**: Browse `https://abfielder.com/Products/BrowseProducts.php?sort=trending&game=minecraft&type=Schematic` — filter by tag (House, Castle, Factory, Bridge, Mega Build, Farm, Medieval, Industrial) — or PhantomMarket's explore page (`market.phantom-node.com/explore`). Planet Minecraft (`planetminecraft.com/projects/tag/litematica/`) and MineMev (`minemev.com/explore/`) have thousands more schematics if you want alternatives. Search for tags: `medieval`, `survival`, `industrial`, `fantasy`, `steampunk`.

**Style guide**: When browsing additional schematics, stick to materials already present in the modpack — stone, deepslate, oak/spruce/dark oak, brick, terracotta, copper, iron, and Create decorative blocks. Avoid schematics that depend on mods not in this pack (looking for Quark, Chipped, or BYG blocks in the material list is a red flag). Medieval, steampunk, dark fantasy, and the modern builds recommended above all fit the Driftwood aesthetic; sci-fi and glass-heavy builds don't.
...
| [MNS - Moog's Nether Structures](https://www.curseforge.com/minecraft/mc-mods/mns-moogs-nether-structures) 3.0.0 | Vanilla-style Nether structures — 40+ ruins, houses, bridges, towers, arenas, Mega Fortress, wrecked train |
...
**MNS - Moog's Nether Structures** is a pure structure mod by finndog_123 (FinnSetchell — same author as Moog's End Structures) that fills the Nether with ~40 vanilla-style structures using only vanilla blocks and entities. Structures include blackstone/nether-brick ruins (circles, pillars, walls, rubble), abandoned houses (medium, large, warped variants), bridges, wells (crimson, warped, lava), towers (copper, nether), warped dome, giant/sandy skulls, sword landmarks, shrines, soul fire, grave yards, a **wrecked train wagon** (campfire + skulls + cracked bricks + hay — a ruin, not a vehicle), and three combat arenas (small, large, dragon) with custom mobs. v3.0.0 adds the **Mega Fortress** — a 207-piece sprawling fortress with loot, traps, dungeons under lava, and a chain-suspended second floor.
...
| [Compass to Map](https://modrinth.com/mod/compass-to-map) 2.0.4                                                   | Auto-creates color-coded JourneyMap waypoints from Explorer's Compass finds — no hand-copying coords         |

### Mobility

| Mod                                                                                         | Role                                                                                                                                                    |
|---------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Immersive Aircraft](https://www.curseforge.com/minecraft/mc-mods/immersive-aircraft) 1.4.6 | Flyable aircraft — biplane, airship, cargo airship, warship, gyrocopter. Upgradable, colorable                                                          |
...
**Companion mod** — Item Descriptions is a **required dependency** of the NeoForge 1.21.1 build: install it alongside Field Guide. (Immersive Overlays removed Sep 2026 — Field Guide runs without it; only the spawn/biome box is lost.) **Mod Descriptions is an optional resource pack** — not a mod: it's language data only (no textures); it's listed in the [Resource Pack Load Order](#resource-pack-load-order) (position irrelevant):
...
All require **YUNG's API** (already in the pack).
...
6. Compass to Map auto-creates the JourneyMap waypoint (color-coded) — no hand-copying coords
...
**Integration with Hybrid Delights (Wave 4)**: HybridLabs' official Farmer's Delight compat adds recipes from Hybrid Aquatic fish and crustacean drops — fish n chips and more in the FD kitchen.
...
**How**: Craft via EMI (iron + wool/leather), place on ground, right-click to enter. `W` to ascend, `S` to descend, `A/D` to rotate. Any furnace fuel works. With the Create Immersive Aircrafts data pack installed (below), recipes switch to Create materials and machines — check EMI for the changed recipes.
...
**Create integration**: The [Create Immersive Aircrafts](https://www.curseforge.com/minecraft/texture-packs/create-immersive-aircrafts-resource-pack) resource pack (3.0) retextures every IA item and vehicle to match Create, and its companion [Recipes data pack](https://www.curseforge.com/minecraft/texture-packs/create-immersive-aircrafts-data-pack) (3.0) rewrites IA crafting, upgrade, and aircraft recipes around Create materials. Install both together — textures and recipes are designed as a pair. The datapack goes in the world's `datapacks/` folder (see `datapacks/README.md`); recipes show in EMI.
...
- **Create recipes companion (optional)** — the [Create Man of Many Planes datapack](https://modrinth.com/datapack/create-man-of-many-planes) (1.1) adds Mechanical Crafter recipes for both aircraft — assemble them on Create machines instead of a crafting table (see `datapacks/README.md`)
...
| [Relics](https://www.curseforge.com/minecraft/mc-mods/relics-mod) 0.12.8 | ~28 levelable relics with abilities, research, random stats — the accessory progression chase |
| [Artifacts](https://www.curseforge.com/minecraft/mc-mods/artifacts) 13.2.5 | Exploration treasure trinkets from chests, archaeology, mimics — permanent utility powers |
| [Reliquified Artifacts](https://www.curseforge.com/minecraft/mc-mods/reliquified-artifacts) 1.0.8 | Compat bridge — folds Artifacts items into Relics leveling/ability systems |
| [Reliquified Twilight Forest](https://www.curseforge.com/minecraft/mc-mods/reliquified-twilight-forest) 0.5.3 | Content bridge — TF-boss-themed relics dropped in the Twilight Forest (TF already in pack) |
...
| Mod                                                                               | Role                                                                                                             |
|-----------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|
| [Improvable Skills](https://www.curseforge.com/minecraft/mc-mods/improvable-skills) 21.0.2 | Buy permanent passive buffs + utility abilities with vanilla XP levels from the Book of Skills |
...
**Improvable Skills** — stock defaults (plug-and-play by design). No overrides authored blind — skill costs are config-adjustable if tuning is ever wanted. [VERIFY] at first launch: XP levels spend on skills; Book of Skills obtainable (see EMI); no conflict with the vanilla XP bar / enchanting table.

### Interdependency notes for Wave 3

- **ShatterLib (OctoLib)** is required by Relics (plus both Reliquified bridges transitively) — Octo-Studios library, new dependency (`OctoLib-NEOFORGE-0.6.2+1.21.jar`, auto-installs via XMCL from the Relics page). Architectury API (already in pack) covers OctoLib's framework expectation.
- **Curios API** provides the accessory slot system used by Relics and Immersive Armors. Adds an equipment GUI (keybind `Y`) with ring, amulet, belt, gloves, back, and charm slots. Auto-installed dependency — no separate config needed. Also satisfies Artifacts' optional Curios compat.
- **Enchantability**: All Simply Swords, Simply More, Too Many Bows, and Immersive Armors pieces are enchantable at the vanilla enchanting table (Easy Magic QoL). Relics/Artifacts accessories are not enchantable (accessory items, by design). Better Combat (Wave 5) is the shared animation layer for the weapon mods.

**Immersive Armors** adds 40+ unique armor sets with custom effects: Wooden (arrow/explosion protection), Skeleton (speed, Wither variant, skeleton neutrality), Divine (dyeable, blocks 1 attack/min), Heavy (high armor, anti-knockback, slow), Slime (bounce attackers), Warrior (berserk — more damage at low HP), Robe (dyeable wool, fire thorns, fireproof), Steampunk (hostile radar, enhanced UI, double jump, no fall damage), and Prismarine (spikes, depth strider). All sets are enchantable, repairable, and dyeable where applicable. Recipes in vanilla recipe book and EMI. All pieces enchant via the vanilla enchanting table (Easy Magic QoL). Mekanism Tools fills the material-tier progression (Osmium→Bronze→Steel→MekaSuit); Immersive Armors fills the unique-effect side grade slot — different armor sets for different situations rather than linear upgrades.
...
**Finding them**: Dungeon chests, mob drops, and boss loot. Enchant them at the enchanting table (Easy Magic QoL) for even more damage.
...
**Version pin (Aug 2026)**: Simply More **1.2.3** (stable, Jun 10, 2026) — pairs with the pinned Simply Swords 1.63.0. The 1.3.0 line is alpha and tied to the Simply Swords 1.70 rewrite; hold until it goes stable.
...
### Relics — Levelable Accessories

**Version pin (Sep 2026)**: Relics **0.12.8** (`relics-1.21.1-0.12.8.jar`) — Octo-Studios/Ilius, NeoForge 1.21.1. ~28 unique relics with abilities, random stats, and research-gated progression. Relics level up with use — the accessory chase that scales across the 200h+ runway.

**Dependencies**: Curios API (already in pack) + ShatterLib/OctoLib 0.6.2 (new) + Architectury API (already in pack).

**Finding strategy**: Use Explorer's Compass → find YUNG's structures → loot them. Relics drop from dungeons, bosses, and structure chests.

### Artifacts — Exploration Treasure

**Version pin (Sep 2026)**: Artifacts **13.2.5** (`artifacts-neoforge-13.2.5.jar`) — ochotonida, NeoForge 1.21.1, MIT. Permanent utility powers (double jump, fire immunity, water breathing, magnet, night vision…) from chests, archaeology, and mimics. All optional-equip — cherry-pick what fits your build.

**Dependencies**: zero required (Curios/Accessories support is optional compat; Curios already in pack covers it).

### Reliquified Artifacts — Compat Bridge

**Version pin (Sep 2026)**: Reliquified Artifacts **1.0.8** (`reliquified_artifacts-1.21.1-1.0.8.jar`) — Octo-Studios, NeoForge 1.21.1. Folds Artifacts items into Relics' leveling/ability systems. Requires Relics + Artifacts (both above). Built against Artifacts 13.2.3; 13.2.4/13.2.5 are crash-fix-only, expected compatible.

### Reliquified Twilight Forest — TF Relic Hunt

**Version pin (Sep 2026)**: Reliquified Twilight Forest **0.5.3** (`reliquified_twilight_forest-1.21.1-0.5.3.jar`) — Octo-Studios, NeoForge 1.21.1. TF-boss-themed relics (Lich Crown, Hydra Heart, Scaled/Chromatic Cloak…) dropped in the Twilight Forest dimension. Requires Relics + Twilight Forest (TF already in pack, Phase 3 endgame dimension).
...
Enchanting stays vanilla-capped (Protection IV, Sharpness V). The gear fantasy now comes from unique weapon/armor passives (Simply Swords, Simply More, Immersive Armors) plus levelable Relics/Artifacts accessories rather than max-level enchantment stacking.

### Improvable Skills — Vanilla-XP Skill Book

The pack's character layer — with no second XP system. Your vanilla levels ARE the skill points: every level earned by mining, fighting, breeding, fishing, crafting… is currency for permanent power stacked on top of the gear chase.

- **Vanilla XP = skill currency** — open the **Book of Skills** (right-click the item; see EMI for how to obtain it) and spend XP levels to permanently unlock skills. One XP pool serves enchanting, anvils, and character growth.
- **Skills** (permanent passive buffs): Extra Health (+20 HP), Strong Fist (melee damage), Sharp Arrow (ranged damage), Attack Speed, Mining / Digging / Cutting (tool speed), Dense Skin (armor), Leaper (jump height), Soft Landing (fall damage), Fast Swimmer, Soul Speed, Huckster (villager discounts), Enchanter (lower enchant level requirement), Experience Harvesting (+XP gain), Accelerated Furnace, Alchemist (brewing speed), Fast Growth, Luck of the Sea, Obsidian Skin, Ender Manipulator, Silent Foot, Treasure of the Sands.
- **Abilities** (utility, crafted as scrolls from Parchment Fragments): portable anvil / crafting table / enchanting table, Magnetism (item magnet), Auto XP Bank (stashes excess levels so you always hold exactly the amount you set).
- **Synergies** — GraveStone Mod keeps your XP on death, so skill-buying power survives too; Easy Magic re-rolls and skills both draw from the same levels, so you choose between enchanting and character upgrades.
- **Requires HammerLib** (Zeith's library mod, auto-installed, pinned 21.0.14). License ARR with explicit modpack permission; 5.3M+ CurseForge downloads.
- **No duplication** — Relics levels gear through use; Improvable Skills buys *character* power with vanilla XP. They layer; they don't overlap. (Magnetism vs Sophisticated Storage's Magnet Upgrade: opt-in ability — skip it if the backpack upgrade already covers you.)
- **Chill-safe** — skills are permanent passive buffs bought at your own pace; no item gating, no mob scaling, nothing hostile to idle play.

The skill GUI has **no keybind** — it lives on the Book of Skills item (right-click to open). There's no dark-theme resource pack for this mod; the GUI renders stock, consistent with other stock-default mods.

### Session Plan — Wave 3

| Session                          | Focus                                                                                                       | What to Do                    |
|----------------------------------|-------------------------------------------------------------------------------------------------------------|-------------------------------|
| **Session 1 — First unique loot** | Check every gear drop and dungeon chest. Look for Simply Swords / Relics / Artifacts finds               | Learn the loot system         |
| **Session 2 — Weapon hunt**      | Explorer's Compass → YUNG's dungeon → loot for a Simply Swords weapon. Try Rapier (fast) or Greathammer (AoE) | First unique weapon           |
| **Session 3 — Enchanting setup**  | Build an enchanting table + bookshelves. Use Easy Magic re-rolls to get Sharpness IV / Protection IV       | First maxed gear              |
| **Session 4 — Relic hunting**    | Loot dungeons and YUNG's structures for Relics + Artifacts trinkets; check the Relics research tree      | Accessory slots filled        |
| **Session 5 — Endgame gear hunt** | Hunt Cataclysm / Twilight Forest / Aether bosses for top-tier unique drops                                 | Endgame gear progression      |
| **Session 6 — Skill book**        | Obtain the Book of Skills (see EMI) and spend XP levels on mining/combat/farming skills as they accumulate | Character progression alongside the gear chase |
...
| [**Hybrid Delights**](https://modrinth.com/mod/hybrid-delights) 1.2.1                                                              | Hybrid Aquatic × Farmer's Delight — HA fish, crustacean & more recipes                                                              |
...
### Hybrid Delights — Hybrid Aquatic Cooking

The official HybridLabs bridge between Hybrid Aquatic (Wave 2) and Farmer's Delight (Wave 4) — FD-style recipes from Hybrid Aquatic's drops: fish and crustacean dishes, plus an FD-style knife (1.2.1 added fish n chips). **Zero new deps** — Farmer's Delight, Hybrid Aquatic, and Kotlin for Forge are all in the pack (Kotlin stays as Hybrid Delights' required dep).

**Integration**: Routes Wave 2's Hybrid Aquatic drops into the Wave 4 kitchen — the crustacean path Ocean's Delight already uses, extended to the full roster. Complements (not duplicates) Ocean's Delight: OD covers generic ocean seafood, Hybrid Delights covers Hybrid Aquatic's specific creatures. Every distinct dish feeds Spice of Life: Carrot Edition's max-health rewards. Recipes show in EMI and store in AE2. Zero config needed — install and cook.
...
| [Cataclysmic Combat](https://www.curseforge.com/minecraft/mc-mods/cataclysmic-combat)                                                              | Better Combat movesets for L_Ender's Cataclysm weapons — gauntlets, Incinerator, Void/Infernal forges, Annihilator, Soul Render               |
...
| [CBC Enchanced Shells [Create Big Cannons]](https://www.curseforge.com/minecraft/mc-mods/cbc-enchanced-shells-create-big-cannons)                  | CBC addon — 86 tiered shells (Mk.I–V), powder charges, doomsday shells, rocket system                                                        |
...
3. Set a Waystone nearby
4. AE2 wireless terminal for mid-fight resupply
...
### Cataclysmic Combat — Better Combat for Boss Weapons

Bridges L_Ender's Cataclysm weapons into Better Combat with proper movesets, so boss drops animate correctly instead of falling back to the vanilla swing:

| Cataclysm weapon    | Better Combat profile |
|---------------------|-----------------------|
| Gauntlet of Guard   | Fist — dual-wieldable |
| Gauntlet of Bulwark | Fist — dual-wieldable |
| The Incinerator     | Claymore — two-handed |
| Void Forge          | Hammer — two-handed   |
| Infernal Forge      | Hammer — two-handed   |
| The Annihilator     | Mace                  |
| Soul Render         | Glaive                |

**Dependency**: Better Combat + L_Ender's Cataclysm (both in pack) — zero new mods. Install the NeoForge 1.21.1 build from CurseForge (the Modrinth listing is the old Forge/1.20.x line).
...
### CBC Enchanced Shells — Tiered Munitions

Massive arsenal expansion for Create Big Cannons — 86 items across 5 progressive tiers (Mk.I → Mk.V) with distinct color codes and a linear upgrade chain:

| Category          | What's added                                                                               |
|-------------------|--------------------------------------------------------------------------------------------|
| Standard shells   | 10 types × 5 tiers (HE, AP, Solid Shot, AP Shot, Shrapnel, Smoke, Drop Mortar, Grapeshot, Mortar Stone, Shell Holder) |
| Powder charges    | 5 upgraded propellants (Mk.I–V) with rising muzzle velocity                                |
| Doomsday shells   | Nuclear Mk.I/II, H-Bomb, Mini H-Bomb (240+ block radius — late-game)                       |
| Shader shells     | 5 exotic projectiles with screen-warping post-process effects                              |
| Rocket system     | Redstone-fired launchpad + cascade-loading breech                                          |

Tiering maps to the progression curve: Mk.I = iron → Mk.V = netherite, spanning Create Phase 2 into Phase 3. Standard-shell explosive power is capped at ×3 for balance.

**Dependency**: Create Big Cannons (above) + Create 6.0+ — both in pack, zero new mods. Install the 1.21.1 NeoForge build from CurseForge.

**Watch — doomsday shells**: nukes are player-fired and opt-in (chill-clean), but a 240+ block radius will flatten your own base. Check the config on first launch and tame/disable the doomsday shells if they feel too destructive for colony defense.
...
| Session                              | Focus                                                                                                                                                                     | What to Do               |
|--------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------|
| **Session 1 — Combat feel**          | Better Combat works automatically. Craft/find a Simply Swords weapon. Test daggers (fast stab) vs greatswords (wide sweep)                                  | Learn your weapon        |
...
| [Macaw\'s Fences and Walls](https://www.curseforge.com/minecraft/mc-mods/macaws-fences-and-walls)   | Fences, gates, and walls in every wood & stone material                                                  |
| [Macaw\'s Lights and Lamps](https://www.curseforge.com/minecraft/mc-mods/macaws-lights-and-lamps)   | Lamps, torches, street lamps, paper lamps in every wood                                                             |
| [Macaw\'s Stairs](https://www.curseforge.com/minecraft/mc-mods/macaws-stairs)             | New stair styles, handrails, balconies                                                                                        |
| [Macaw\'s Paintings](https://www.curseforge.com/minecraft/mc-mods/macaws-paintings)       | Custom hand-drawn paintings for your walls                                                                                    |
...
| [Create Deco](https://www.curseforge.com/minecraft/mc-mods/create-deco) 2.1.3             | Industrial decoration — catwalks, train hulls, sheet metal, shipping containers, cage lamps, brick sets, decals               |
...
Eight modules covering structural elements in every wood type.
...
**Fences & Walls**: Picket, stockade, horse, wired, and highley fences + matching gates, plus stone/brick walls. Each a distinct look.

**Lights and Lamps**: Vanilla-styled lamps, torches, street lamps, paper lamps, and wall lamps — one for every wood type. Craft with matching wood + glowstone/lantern material.

**Stairs**: New stair styles (Terrace, Skyline, Compact, Bulk, Loft), handrails for stairs, and balconies/platforms.

**Paintings**: Custom hand-drawn paintings to fit your world — 35 new in 1.1.0 (farm scenes, the Creaking, pets, coral reefs) + 22 updated.
...
Industrial decoration for Create-native builds — from catwalks to shipping containers.

- **Block sets** in 6 metals (andesite, zinc, iron, industrial iron, copper, brass): support blocks/wedges, train hulls, catwalks + railings + stairs, cage lamps (4 light colors), sheet metal, windows/panes, bars, chain-link fences, coin stacks, doors, ladders
- **Shipping containers** — all 16 dye colors, function like Create Item Vaults with a fresh look
- **Bricks** — 8 colors (red, scarlet, blue, umber, dean, dusk, verdant, pearl), each with stairs, slabs, and cut variants
- **Decals** — hazard / restricted-area signs

**Placement**: Decoration available from Phase 1 materials (iron/zinc/copper/brass). Pairs with Create (Wave 1) and Rechiseled: Create for industrial detailing. All recipes in EMI; no config needed.
...
**Integration with Wave 3 gear**: Glass cloches and katana stands are ideal for showcasing unique drops. Mount your best Simply Swords / Relics / Artifacts finds as trophies. Wall holders with plaques let you name your legendary weapons — "Flamebane, Bane of the Ender Dragon."
...
| **Session 4 — Details**             | Supplementaries jars, signposts, weather vanes. Laser Bridges & Doors for energy barriers and decorative lighting. Rechiseled: Create for factory aesthetics | Decorative details       |
...
2. **Optional data packs** — for the structure-spacing tweak, place `rarer-better-dungeons/` in `saves/<world>/datapacks/` after creating the world (see `datapacks/README.md`).
   The Create aircraft datapacks (`Create Immersive Aircrafts Datapack 1.21.1 - 3.0.zip` + `Create-Man-of-Many-Planes-1.1.zip`) go in the same folder to enable Create-machine aircraft crafting.
...
**Game Rules → Chat → disable all options** (Command Feedback, Command Block Output, Reduced Debug Info, Show Death Messages, Show Recipe Messages, Announce Advancements). Keeps the chat clean — progression popups still appear via Fancy Toasts.
...
|----------------------------|---------|--------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Wave 0 — Foundation        | 10      | 10     | 21      | Dependencies (12), Performance & Rendering (12) + FastWorkbench −Sodium Extras −Sodium Options API (Aug 2026 — broken dep chain: sodiumoptionsapi→RSO→Sodium 0.8.12+→incompatible with sodiumoptionsapi), Infrastructure (3) + Simple Recall Potion (travel) — all the nuts & bolts that make the game run (+1 shaderpack, +26 resource packs). Wave 0.5 UI swap: −Immersive UI −Smooth GUI −Tooltip Overhaul −Controlling −First Person Model +Modern UI; Aug 2026 Wave 0.5 review: −Easy Anvils (hard conflict with anvil mixins) −Searchables (orphan library — does nothing standalone) +Prism (Item Borders required dep) +Puzzles Lib (dep documented — 5 consumers) +Dark Mode Everywhere→Mindful Darkness swap +Stylish Effects −Status Effect Bars Reforged (Aug 2026 swap — SEBR's duration bars natively covered) −Dynamic Crosshair (vanilla-coupled — context features unreliable on modded worlds), Aug 2026 −JadeColonies (moved to Wave 1.5), Aug 2026; −Traveler Tool Belt (removed per user request, Aug 2026); −EMIffect +JEED +MEED (effect-wiki swap + curated content, Aug 2026); −Mindful Darkness → Dark Coffee GUI + Modded Coffee GUI resource packs (full swap, Aug 2026 — GUI dark mode is now a resource pack, not a mod); +Fast Noise (replaces NoisiumForked — archived predecessor, Aug 27 2026); +C2ME (NeoForge) — parallel chunk gen, pairs with Fast Noise + Lithium, alpha backport flagged (Aug 27 2026); −Immersive Armor HUD +Detail Armor Bar Reconstructed, Aug 2026; −Advancement Plaques −Toast Control +Fancy Toasts +Fancy Notify, Aug 29 2026 (2:2 swap, counts unchanged); +Grassier Grass 1.4.5 (Sep 2026 — Wave 0 Visual QoL, animated grass blades, client-only, zero deps); −Countered's Smooth F5 (removed per user request, Sep 2026 — BTP keeps third-person rotation, zero deps removed); +JustEnoughBreeding 3.2.1 (Sep 2026 — EMI breeding lookup, client-only, zero deps); −Inventory Essentials (removed per user request, Sep 2026 — ClientSort keeps sorting; transfer gestures lost, Balm stays for Waystones)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Wave 0.5 — Resource Packs  | 5       | 0      | 5       | EMF + ETF + Continuity + Sinytra Connector + Cull Leaves — client-side support mods for Fresh Animations (EMF/ETF), Overlay's (Continuity/Connector), Better Leaves perf (Cull Leaves); +11 resource packs −Patrix +Faithful −FA Extensions (Aug 2026 resource pack review; removed — Details addon corrupted animal models, user report) −Armored Legacy (removed per user request, Sep 2026)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Wave 1 — Tech              | 20      | 5      | 25      | Create + 12 addons (incl. Power Grid, Gunsmithing, Protection Pixel, Ornithopter Glider, Train Utilities, Railways Navigator, Interiors, Train Parts, Central Kitchen), Mekanism + Generators + MekaJadeUpgrades, AE2, Advanced Finders, Advanced Chimneys + NTGL, GeckoLib, Framework, ForgeEndertech, Kleiders deps, Mekanism: Weapons (Phase 3 energy weapons — polonium/atomic-alloy gated)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Wave 1.5 — Colony          | 6       | —      | 6       | MineColonies + 5 addons: Byzantine Styles, Stylecolonies, Create: Colony Logistics, ColonyLink, JadeColonies |
| Wave 2 — Exploration       | 37      | 7      | 45      | YUNG's (12), Terralith/Tectonic, Serene Seasons + Serene Seasons Plus + Gabou's Libs, Darker Depths, Upgrade Aquatic, dimensions (3), End overhaul (3 + Nullscape dp + 5 deps), navigation, aircraft, ships, hang glider, MoMP addon, Vehicle Upgrade (mount/boat QoL), Incendium (Nether biome overhaul), **MNS (Nether structures)**, Immersive Machinery (utility machines), Field Guide + Item Descriptions (required companion), Bumblezone (bee dimension) + Resourceful Lib, Streams Reflowing (hydrology — carved streams/lakes + river currents, Terralith/Tectonic first-class; restores the Dynamic Waters role removed Aug 27) +Compass to Map 2.0.4 (Sep 2026 — auto JourneyMap waypoints from Explorer's Compass finds, zero required deps)
| Wave 3 — Equipment Magic   | 10      | 7      | 17      | Simply Swords, Simply More, Too Many Bows, Relics, Artifacts, Reliquified Artifacts, Reliquified Twilight Forest, Immersive Armors, Easy Magic + 7 deps (Placebo, Patchouli, Simply Tooltips, Fzzy Config, ShatterLib, Curios API, HammerLib) — −Project MMO −PMMO × Farmer's Delight Compat (Aug 27 2026 — reverted; not what the user wanted) −Skills Mastery Reimagined −Pufferfish's Skills −Pufferfish's Attributes (Aug 27 2026 — skills dropped; no tech-vibe tree, user decision) −Apotheosis −Apothic Compats −3 Apothic modules +Easy Magic (Aug 26 2026 removal), pins SS 1.63.0 + SM 1.2.3 +Tyz's Skills→Improvable Skills (Aug 31 2026 — skills restored then swapped same-day: Improvable Skills 21.0.2 + HammerLib 21.0.14 — vanilla-XP skill book replaces Tyz's separate XP system per user request)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Wave 4 — Food & Farming    | 20      | —      | 20      | Farmer's Delight + 9 addon mods + Neo Bee Fix + Comfortable Campfires + Smarter Farmers + Create: Café + Starcatcher + Starcatcher's Delight + Better Fishtanks + Spice of Life: Carrot Edition + Crops Love Rain + Hybrid Delights (Aug 2026 review; count corrected 14→18 — fishing trio + one Delight addon never reflected in summary)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Wave 5 — Combat & Mobs     | 8       | 5      | 13      | Better Combat, L_Ender's Cataclysm, Enchantment Descriptions, Create Big Cannons + Advanced Technologies addon, Torchmaster, CBC Enchanced Shells, Cataclysmic Combat |
| Wave 6 — Building          | 15 | 3 | 18 | Rechiseled, Supplementaries, Macaw's (8), Handcrafted, Fetzi's Displays, Lili's Pottery, Laser Bridges & Doors, Diagonal Fences + Create Deco + Rechiseled: Create, Rechiseled: AE2 + Moonlight, Resourceful, Fusion −Building Wands (removed per user request, Aug 28 2026)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                …
...
| **Total**                  | **182** | **43** | **225** | All confirmed NeoForge 1.21.1 (Aug 2026 Wave 0.5 review: −Easy Anvils −Searchables +Prism +Rhino; Dark Mode Everywhere→Mindful Darkness swap + Puzzles Lib dep documented, Aug 2026; −Stellaris −Potentials API −Sky Aesthetics −CC:Tweaked +Northstar Redux, Aug 2026; +Real Camera −Real Camera (removed per user request, Sep 2026 — mods −1, deps unchanged); Day Counter Plus reverted — Day Counter + Hud Texts restored; +BetterGrassify + Forgified Fabric API + Sky Aesthetics; +Serene Seasons Plus + GlitchCore + Gabou's Libs - Chunky - When Dungeons Arise - Structory: Towers, Aug 2026; +Smarter Farmers (Sodium Extras + Sodium Options API removed Aug 2026 — broken dep chain on NeoForge 1.21.1) + Diagonal Fences + Create: Café, Aug 2026 mod review; +Arsenal (RPG Series) + Shield API, Enchanting Runes dep corrected Bundle API→FLIB, stable pins SS 1.63.0 + SM 1.2.3, classes TODO closed (Aug 25 2026 Wave 3 review); −Enchanting Runes −FLIB (duplicates the pack's socketing role; unproven 1.21.1), Aug 25 2026; −Mindful Darkness → Dark Coffee GUI + Modded Coffee GUI resource packs (full swap, Aug 2026 — mods −1, deps unchanged, +2 resource packs → 24); +Fa…; −Apotheosis −Apothic Compats −3 Apothic modules −Loot Beams −Nirvana Library −Common Network +Easy Magic (Aug 26 2026); −Dynamic Waters (worldgen — falling off the world, user report, Aug 27 2026); +Fast Noise (replaces NoisiumForked — archived predecessor, Aug 27 2026); +C2ME (NeoForge, parallel chunk gen — alpha backport, Aug 27 2026); −Skills Mastery Reimagined −Pufferfish's Skills −Pufferfish's Attributes (Aug 27 2026 — skills dropped; no tech-vibe tree, user decision); −Project MMO −PMMO × Farmer's Delight Compat (Aug 27 2026 — reverted; not what the user wanted) +CBC Enchanced Shells +Cataclysmic Combat (Aug 28 2026 — Wave 5 review) +Macaw's Fences and Walls (Aug 28 2026 — Wave 6 5th Macaw's module) −Building Wands (removed per user request, Aug 28 2026) +Macaw's Lights and Lamps +Macaw's Stairs +Macaw's Paintings (Aug 28 2026 — Wave 6 6th–8th Macaw's modules, all zero-dep); −Immersive Armor HUD +Detail Armor Bar Reconstructed, Aug 2026 (1:1 swap, zero deps, counts unchanged); −Advancement Plaques −Toast Control +Fancy Toasts +Fancy Notify, Aug 29 2026 (2:2 swap, counts unchanged); +Tyz's Skills + Tyz's Skills Dark Theme, Aug 31 2026 (skills restored — zero-dep; +1 mod, +1 resource pack → 25) → swapped Aug 31 to Improvable Skills + HammerLib −Tyz's Skills Dark Theme (vanilla-XP skill book per user request; deps +1 → 43, resource packs 25 → 24); +Streams Reflowing (Sep 2026 — Wave 2 hydrology, zero deps); +Upscaled Mekanism 32x v1.5 (Sep 2026 — Mekanism 32x layer, zero deps, resource packs 24 → 25); +Grassier Grass 1.4.5 (Sep 2026 — Visual QoL animated grass blades, client-only, zero deps); −Countered's Smooth F5 (removed per user request, Sep 2026 — mods −1, deps unchanged); +AttributeFix (Sep 2026 — Wave 0.5 bar engine, lifts vanilla attribute caps for Overflowing Bars; +Prickle +Bookshelf deps); −Real Camera (removed per user request, Sep 2026); Dark Loading Screen Neoforge 1.0.1 trialed and reverted Sep 2026 (user report: doesn't theme the initial/early loading screen — DLS restored, counts unchanged); +JustEnoughBreeding 3.2.1 (Sep 2026 — EMI breeding lookup, client-only, zero deps; mods +1, deps unchanged); +Compass to Map 2.0.4 (Sep 2026 — compass→JourneyMap glue, zero required deps; mods +1, deps unchanged); −Inventory Essentials (removed per user request, Sep 2026 — mods −1, deps unchanged); −TFMG Energy Converter (removed per user report, Sep 2026 — broken; mods −1, deps unchanged — Kotlin for Forge stays for Hybrid Delights); −Arsenal (RPG Series) −Relics (RPG Series) −Spell Engine −Ranged Weapon API −Shield API +Relics 0.12.8 +Artifacts 13.2.5 +Reliquified Artifacts 1.0.8 +Reliquified Twilight Forest 0.5.3 +ShatterLib (Sep 2026 — RPG pair swapped for Ilius Relics + Artifacts stack per user request; mods +2, deps −2, total unchanged); −Immersive Overlays (removed per user request, Sep 2026 — Field Guide companion only, HUD role duplicated by JourneyMap/BetterF3/Traveler's Titles; Field Guide runs without it, loses only the spawn/biome box; mods −1, deps unchanged) |……
...
| Bridge TFMG oil to AE2            | TFMG diesel → SU → Crafts & Additions alternator → Mekanism universal cable                                              |
...
| Build a megabase                  | Create Blueprint + Schematicannon (structure) + Rechiseled blocks (aesthetic) + Handcrafted furniture (interior) |
