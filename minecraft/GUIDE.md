...
| [Super Resolution](https://modrinth.com/mod/superresolution) 1.21-0.9.1-alpha.1   | FSR/DLSS/XeSS/SGSR upscaling + frame generation — render below native, upscale to target (client-only, zero deps) |

**ModernFix** (configurable in-game via Mod List → ModernFix → Config, or `config/modernfix-mixins.properties`):

**Super Resolution** (client-only, zero deps) — AMD FSR 1/2/3, NVIDIA DLSS, Intel XeSS, Snapdragon SGSR upscaling + frame generation. Renders below native resolution and upscales to your target — the rendering-side headroom lever for the 4K shader + PBR workload once the shader-side levers (Shadow Resolution → Volumetric samples) are spent. Pin **1.21-0.9.1-alpha.1+gl-neo** (Modrinth). ⚠ The 1.21.1 line is **alpha** — verify on your GPU before relying on it: needs OpenGL 4.3+, Windows/Linux (no macOS), and downloads its DLSS/FSR resource DLLs on first use. Explicitly compatible with Sodium, Iris, and Distant Horizons (all in pack; dedicated 1.21.1 Sodium 0.8.x compat fix). SR ratio >1.0 = FPS boost (2–3× typical), <1.0 = high-quality anti-aliasing.
