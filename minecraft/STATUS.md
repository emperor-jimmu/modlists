...
- [ ] **Decision needed**: adopt the conditionals (KubeJS Mekanism, Mekanism: Weapons) and/or Covers? Adoption updates GUIDE.md (Wave 1 Mekanism Addons rows + Mod Count Summary) and this entry; rejection is final as recorded.
- [ ] **Instance action (XMCL, only for adopted mods)** — add `mekanism-covers` jar + `config/mekanismcovers.json` (`disableAdvancedCoverRendering = true`); `kubejs-mekanism-neoforge-2101.1.7-build.18.jar`; Mekanism Weapons jar to `mods/`. [VERIFY] at next launch: no missing-dependency errors; covers render (no crash with Iris); Mekanism recipe handlers appear in KubeJS logs; weapon damage sane vs Apotheosis mythic gear.
