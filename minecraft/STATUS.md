...
- [ ] **Instance action (XMCL)** — remove Patrix 32x pack; add the 22 packs per GUIDE.md §Resource Pack Load Order; add 5 jars (EMF, ETF, Continuity, Sinytra Connector, Cull Leaves). [VERIFY] at next launch: Armored Legacy armor trims render, Overlay's moisture/crack transitions on dirt/farmland, FA Extensions Emissive glows, connected fire/soul fire via Rekindled CTM, AppleSkin food bar at 32x, Field Guide modded entries show lore, no resource-pack incompatibility warnings, no crashes from Connector.
- [x] DRIFTWOOD-GUIDE.pdf regenerated
