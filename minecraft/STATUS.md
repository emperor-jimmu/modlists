### Reliquified Bridges Dropped, Accessories Added (Sep 24, 2026)

- [x] **User request**: remove Reliquified Artifacts + Reliquified Twilight Forest; add Accessories (`accessories`, 1.1.0-beta.53+1.21.1, NeoForge 1.21.1, MIT, verified live via Modrinth API) as Artifacts' second slot backend alongside Curios. Required dep: oωo (owo-lib) 0.12.15.5-beta.1+1.21 (auto-installs via XMCL). Note: the 1.21.1 Accessories line is beta-channel — latest available, no stable exists.
- [x] **GUIDE.md updated**: Equipment table bridge rows → Accessories row; bridge sections replaced with an Accessories section + pin; Artifacts/Curios dep notes rewritten (dual-backend wording); Mod Count Summary Wave 3 10/7/17 → **9/8/17**; Total **182/43/225 → 181/44/225** (mods −1, deps +1, total unchanged). Relics/Artifacts/Improvable Skills untouched; ShatterLib stays (Relics needs it); Curios stays (Relics + Immersive Armors).
- [ ] **Instance action (XMCL)** — remove `reliquified_artifacts-*.jar` + `reliquified_twilight_forest-*.jar` from `mods/` (do not add — supersedes the Sep 24 RPG-swap instance action above); add `accessories-neoforge-1.1.0-beta.53+1.21.1.jar` (+ `owo-lib-neoforge-0.12.15.5-beta.1+1.21.jar` auto-installs). [VERIFY] at next launch: no missing-dependency errors; Artifacts trinkets equip via Curios or Accessories menu.
- [x] DRIFTWOOD-GUIDE.pdf regenerated

- [x] **User request**: replace Arsenal (RPG Series) + Relics (RPG Series) with Relics (`relics-mod` 0.12.8) + Artifacts (13.2.5) + Reliquified Artifacts (1.0.8) + Reliquified Twilight Forest (0.5.3). All four pins verified live via Modrinth API (NeoForge 1.21.1). Note: `relics-mod` (Ilius/Octo-Studios) and `relics-rpg` (RPG Series) are different mods by different authors — no relation.
- [x] **GUIDE.md updated**: Equipment table rows swapped; RPG-Series sections replaced with Relics / Artifacts / Reliquified Artifacts / Reliquified Twilight Forest sections + pins; dep notes rewritten (Spell Engine + Ranged Weapon API + Shield API out — orphaned, sole consumers were the RPG pair; ShatterLib/OctoLib 0.6.2 in as the one new dep; Curios API stays for Relics + Immersive Armors); session plan + gear-fantasy + MEED + Fetzi's trophy lines de-RPG'd. Artifacts needs zero new deps (Curios compat optional); RTF rides on Twilight Forest (already in pack); GeckoLib/Patchouli/HammerLib/Simply Tooltips/Fzzy Config unaffected. Mod Count Summary Wave 3 8/9/18 → **10/7/17**; Total **180/45/225 → 182/43/225** (mods +2, deps −2, total unchanged).
- [ ] **Instance action (XMCL)** — remove `arsenal-*.jar`, `relics-rpg-*.jar`, `spell-engine-*.jar`, `ranged-weapon-api-*.jar`, `shield-api-*.jar` from `mods/`; add `relics-1.21.1-0.12.8.jar` + `artifacts-neoforge-13.2.5.jar` + `reliquified_artifacts-1.21.1-1.0.8.jar` + `reliquified_twilight_forest-1.21.1-0.5.3.jar` + `OctoLib-NEOFORGE-0.6.2+1.21.jar` (Curios + Architectury already present). [VERIFY] at next launch: no missing-dependency errors; Relics research/leveling opens; Artifacts trinkets equip via Curios slots; TF relics drop in Twilight Forest.
- [x] DRIFTWOOD-GUIDE.pdf regenerated
...
- [x] **Pinned LambDynamicLights 4.8.11+1.21.1** in the GUIDE table row (previously unversioned; client-only, zero new deps).
- [x] **User request: add 3 FA packs → adopted 2, skipped the bundle**. **Objects v2.1.2** (`FA+Objects-v2.1.2.zip`, Jun 23 2026, 1.21.1 ✅, `dependencies: []`, ~1.6M DL — animates chests/boats/minecarts/shulkers, genuinely new coverage) + **Emissive v1.6.0** (`FA+Emissive-v1.6.zip`, Apr 27 2026, 1.21.1 ✅, `dependencies: []`, ~1.5M DL — emissives for Drowned/Stray/Bogged/Golem/Guardians/firing Ghast/angry Bee+Wolf/Zombie Villager/Piglins/Hoglins/Bat; rendered via ETF, lit via LambDynamicLights). **Skipped Extensions bundle** (21.5M DL but drags 6 unasked packs — creeper/spider/horse remodels that fight the 32x direction; à la carte wins). All official FreshLX, ARR pack-include-allowed, XMCL install. 16x-vanilla styling inherits the already-accepted FA tradeoff. Load both directly above Fresh Animations (author requirement).
- [x] **GUIDE.md updated**: Lamb row pinned; Texture Packs table +2 rows; blurbs (+Objects/+Emissive, ETF+Lamb pairing note); Resource Pack Load Order 25 → **27** (FA #8, Objects #9, Emissive #10, rest renumbered, Faithful base now #27); Wave 0 note `+25 → +27 resource packs`. No mod/config/keybinding/datapack changes.
- [ ] **Instance action (XMCL)** — pin `lambdynamiclights-4.8.11+1.21.1.jar`; add `FA+Objects-v2.1.2.zip` + `FA+Emissive-v1.6.zip` to `resourcepacks/` enabled directly above Fresh Animations. [VERIFY] at next launch: chests/boats/minecarts animate; glowing mobs render emissive (ETF) and cast light (Lamb); no pack-order warnings.
- [x] DRIFTWOOD-GUIDE.pdf regenerated
...
- [x] **Config override** (spec §4 intent, captured from first launch): `config/streamsreflowing/` committed to the repo after Task 4's launch — streams/lakes ON, river currents ON, `rebiomeStreams` default, quality preset default-or-one-notch-down (decided at launch), particle budget tuned only if frames hurt under DH+Iris, Terralith Yellowstone carve-blacklist confirmed present, chat welcome message OFF. 2.13.1's generated defaults already satisfy spec §4 — committed as-is with ZERO value changes: streams/lakes ON, river currents ON (`vanillaWaterFlow` + `flowVanillaRivers`), `rebiomeStreams=1`, `spillOnStreamsOnly=true`, `welcomeMessage=false` (OFF — pack clean-chat; sole deviation from stock default), Yellowstone confirmed in `streamBiomeBlacklist`, terrain accuracy MEDIUM (default) kept (launch smooth), particle budget at defaults (frames fine). Files: `config/streamsreflowing-common.toml` + `config/streamsreflowing-client.toml` (full generated files, tracked). `bank_features/`/`bank_styles/` seeded defaults NOT tracked (no pack customization).
- [x] **Instance action (XMCL)**: add `StreamsReflowing-1.21.1-neoforge-2.13.1.jar` from Modrinth (`streams-reflowing`) to `<instance>\mods\` — instance `Driftwood` at `C:\Users\nimro\.minecraftx\instances\Driftwood`. No `dynamicwaters*.jar` present (Aug 27 removal action moot — do not add). [VERIFY] at next launch (fresh world): no missing-dependency errors; streams/lakes generate following terrain; natural rivers flow one direction to the sea; Create water wheel in a stream turns; boat drifts downstream and can move against the current; no unsafe-terrain-read log spam; no chunk-border artifacts; Terralith Yellowstone has no carved streams. User launch test (fresh world, Sep 2 2026): **PASS** — "it works great". Streams/lakes generate, rivers flow to the sea, Create water wheel turns on stream water, boat drifts/upstream works, no log spam or chunk-border artifacts, Terralith Yellowstone un-carved.
- [x] DRIFTWOOD-GUIDE.pdf regenerated

### Tyz's Skills → Improvable Skills Swap — Vanilla-XP Character Layer (Aug 31, 2026)

- [x] **User request**: Tyz's Skills is "close to what I want, but still runs on a separate leveling/XP system" — replace it with a skill mod that runs on **vanilla Minecraft XP/levels**. Improvable Skills 3 ([CurseForge](https://www.curseforge.com/minecraft/mc-mods/improvable-skills)) chosen; user confirmed (noted 5.3M+ CF DLs; 1.8-yr-stale 1.21.1 build = acceptable).
- [x] **Research** (Modrinth API + CurseForge + GitHub + official wiki, Aug 31 2026) — vanilla-XP candidates on NeoForge 1.21.1: **Improvable Skills** (spend vanilla XP levels on skills; CF description + source verified), **Bloxy's Skills** (vanilla XP → skill-point conversion; MCreator, v1.0.0-only), **Nasir's Simple Skills** (datapack; 30 levels → 1 skill point). Rejected as non-vanilla-XP: **GokiSkills** (GokiStats port — per-skill XP by use), **LevelZ** (Fabric-only per official wiki "no plans for Forge", own LevelZ XP via JobsAddon, gates gear/items → chill-rule conflict), **Pufferfish's Skills** ecosystem (own per-category XP/points framework + 2 extra mods), Aura / Prodige's (own XP systems).
- [x] **Pinned Improvable Skills 21.0.2** (`ImprovableSkills-1.21-21.0.2.jar`, 1.2 MB, Sep 2 2024, release; Modrinth API + CurseForge verified — CF file 5685129, 261.5K DL on this file) — last NeoForge 1.21.1 build of Zeith's PlayerStats rewrite ("upgrade yourself using experience and Book of Skills"); project **5.3M+ CF DL** / 202K Modrinth DL, **ARR** (explicit "you can use this mod in your modpack" permission), author Zeith (DragonForge, 139M CF DL). **Required dep: HammerLib**; optional JEI integration skipped (pack is EMI-only).
- [x] **Pinned HammerLib 21.0.14** (`HammerLib-1.21-21.0.14.jar`, 2.1 MB, Sep 20 2024; Modrinth API + CurseForge verified — CF file 5739162, 1.0M DL on this file) — last 1.21.1 NeoForge build of Zeith's library; 57.1M CF project DL, zero deps, ARR.
- [x] **What it does** (CF description + 1.21 source verified): **Book of Skills** item → right-click opens the skill GUI (`ItemSkillsBook.use` → skills screen; **no keybind** — item-driven UI). Spend **vanilla XP levels** on permanent passive skills (Extra Health +20 HP, Strong Fist, Sharp Arrow, Attack Speed, Mining/Digging/Cutting speed, Dense Skin, Leaper, Soft Landing, Fast Swimmer, Soul Speed, Huckster, Enchanter, Experience Harvesting, Accelerated Furnace, Alchemist, Fast Growth, Luck of the Sea, Obsidian Skin, Ender Manipulator, Silent Foot, Treasure of the Sands) + crafted ability scrolls (portable anvil / crafting / enchanting, Magnetism, Auto XP Bank).
- [x] **Fit**: same character-layer role as Tyz's (permanent passive buffs layered on the gear chase) but the currency is the XP the player already earns — one XP pool for enchanting, anvils, and skills (integration rule); GraveStone keep-XP makes death forgiving for the skill currency too; reward-first, no item gating (chill rule); no mob scaling. Relics levels gear, Improvable Skills buys *character* power — no duplication.
- [x] **Watch item** (documented in GUIDE.md): **Magnetism** ability duplicates Sophisticated Storage's Magnet Upgrade — opt-in, just don't buy it; config-adjustable if it ever matters.
- [x] **Dark Theme resource pack removed** (`Tyz's Skills Dark Theme 6.4 - 1.21.1.zip`) — no Improvable Skills theme exists; the skill GUI renders stock (consistent with other stock-default mods). Supersedes the Aug 31 Tyz's texture-pack adoption.
- [x] **GUIDE.md updated**: Wave 3 RPG Progression table row (−Tyz's Skills +Improvable Skills 21.0.2); "Tyz's Skills — Passive Skill Growth" section → "Improvable Skills — Vanilla-XP Skill Book"; Configuration — Wave 3 bullet; Interdependency note (+HammerLib 21.0.14, −zero-dep/KubeJS claim); Session Plan Session 6 rewritten; Keybindings `I` row removed (no keybind — item-driven); Texture Packs table row + blurb + Resource Pack Load Order #24 removed (Faithful 32x 25→24). Mod Count Summary: Wave 3 `8/8/17 → 8/9/18`; Total `180/42/222 → 180/43/223` (1:1 mod swap, deps +1); resource packs 25 → **24** (Wave 0 note + Total changelog).
- [ ] **Instance action (XMCL)**: remove `Tyz's skills 6.4.2 - 1.21.1.jar` from `mods/` and `Tyz's Skills Dark Theme 6.4 - 1.21.1.zip` from `resourcepacks/` (the Aug 31 Tyz's instance action is superseded); add `ImprovableSkills-1.21-21.0.2.jar` (CurseForge `improvable-skills`, file 5685129) + `HammerLib-1.21-21.0.14.jar` (CurseForge `hammer-lib`, file 5739162) to `mods/`. [VERIFY] at next launch: no missing-dependency errors; Book of Skills obtainable and right-click opens the skill GUI; spending XP levels buys skills; skills persist after death (GraveStone keep-XP); `I` opens nothing (Tyz's keybind gone); no clash with Relics/Curios/EMI screens; no leftover tyzs_skills config errors.
- [x] DRIFTWOOD-GUIDE.pdf regenerated

### Tyz's Skills + Dark Theme Adopted — Skills Restored to Wave 3 (Aug 31, 2026)

- [x] **User request: add Tyz's Skills** ([CurseForge](https://www.curseforge.com/minecraft/mc-mods/tyzs-skills)) + its **Dark Theme texture pack** ([CurseForge](https://www.curseforge.com/minecraft/texture-packs/tyzs-skills-dark-theme)). This is an explicit revisit of the Aug 27 skills-drop decision ("every other tree… Tyz's Skills… is generic MMO/fantasy") — the user's direct request supersedes the skip.
- [x] **Pinned 6.4.2** (`Tyz's skills 6.4.2 - 1.21.1.jar`, 799 KB, May 20 2026, release; Modrinth API verified) — current NeoForge 1.21.1 line; project **~199.2K CurseForge DL** / 22.9K Modrinth DL, **MIT**, active (26.1 port in progress). **Zero dependencies** (`dependencies: []`). CurseForge file 8118598 matches (same date/line).
- [x] **What it does** (verified against 6.4.x changelogs + official GitBook docs — the store-page blurbs are stale, still advertising the Traits system removed in 6.3.0): passive XP from basic actions (mining, fighting, breeding, eating, fishing, crafting, sleeping, biomes, advancements) → levels → Skill Points (bonus every 5 levels) → permanent passive buffs (speed/damage/resistance/XP boosts…); level requirements + skill conflicts (JSON), bulk buy/refund (Shift-click), refunds at 30% (configurable), death penalties configurable (OFF by default). **99% data-driven** (`config/tyzs_skills/` JSON) + **fully KubeJS-compatible** (Wave 1 KubeJS already in pack — zero new frameworks).
- [x] **Fit**: restores the pack's character-progression layer — Relics levels gear via use, Tyz's Skills levels the *character* (no duplication); reward-first, no item gating (chill rule); no mob scaling, buffs small/passive (easy-curve preserved); every wave's actions feed one passive power curve (integration rule). Wave 3 — RPG Progression (the table deleted with Skills Mastery Aug 27 is restored).
- [x] **Dark Theme resource pack pinned 6.4** (`Tyz's Skills Dark Theme 6.4 - 1.21.1.zip`, May 19 2026; Modrinth API verified) — "Only compatible with Tyz's Skills 6.4+" (covers pinned 6.4.2); official companion by tyzbrand; 16x dark GUI theme; client-side, zero deps, All Rights Reserved (same license class as Dark Coffee GUI); 1.8K CF / 928 Modrinth DL. Complements the pack's dark-UI direction (Dark Coffee GUI swap precedent).
- [x] **Keybind**: `I` opens the skills menu (official FAQ verified; also an inventory-screen button) — free in the pack's keybind table. Added to Keybindings Reference.
- [x] **GUIDE.md updated**: Wave 3 +`### RPG Progression` table (restored) + "Tyz's Skills — Passive Skill Growth" section; Configuration — Wave 3 bullet; Interdependency note (zero-dep + KubeJS); Session Plan +Session 6; Keybindings row (`I`); Texture Packs table row + blurb + Resource Pack Load Order #24 (Faithful 32x → #25). Mod Count Summary: Wave 3 `7/8/16 → 8/8/17`; Total `179/42/221 → 180/42/222` (mods +1, deps unchanged); resource packs 24 → **25** (Wave 0 note + Total changelog).
- [ ] **Instance action (XMCL)**: add `Tyz's skills 6.4.2 - 1.21.1.jar` to `mods/` (CurseForge `tyzs-skills`, file 8118598); add `Tyz's Skills Dark Theme 6.4 - 1.21.1.zip` to the instance's `resourcepacks/` and enable it above the Faithful 32x base. [VERIFY] at next launch: no missing-dependency errors; `I` opens the skills menu; XP accrues from mining/fighting/crafting; death penalties OFF (`config/tyzs_skills/`); no clash with Relics/Curios/EMI screens; dark theme renders on the skills GUI.
- [x] DRIFTWOOD-GUIDE.pdf regenerated
...
- [x] **Spec + plan**: `docs/superpowers/specs/2026-08-29-swap-immersive-armor-hud-design.md`, `docs/superpowers/plans/2026-08-29-swap-immersive-armor-hud.md`.
...
### Subtle Effects Re-review — SKIP (already replaced by Particular Reforged) (Aug 28, 2026)

- [x] **User asked: should we re-add [Subtle Effects](https://modrinth.com/mod/subtle-effects)?** Modrinth API re-verified (Aug 28 2026): project approved/active, **10.4M DL**, updated Jun 17 2026; NeoForge 1.21.1 line current at **1.14.3** (Apr 29 2026, release). **SKIP — do not re-add.**
- [x] **Recorded precedent decides this**: Subtle Effects was removed Jul 2026 in the **1:1 swap → Particular Reforged** (STATUS: "Added Mods Jul 2026" — "replaces Subtle Effects, same category" / "Removed Mods Jul 2026 batch 2"), and that swap is the pack's canonical no-duplicate-core-purpose precedent (cited again in the Stylish Effects entry). Re-adding reverses a recorded decision with no new information.
- [x] **Same lane, partially overlapping**: both are ambient-particle-detail mods in Wave 0 Visual & Client QoL. Particular (installed) owns 3D splashes, waterfall cascades, leaf particles, rain/water-drip ripples, cave dust, underwater bubbles, lava drips — all toggleable. Subtle's list (verified from README): fire-block sparks, snoring Z-particles, slime trails, glowstone/redstone dust motes, allay magic, sheep-shear fluff, burning-entity sparks, campfire-style smoke, heartbeat/stomach-growl at low stats, mob-head spectator shaders, running/landing dust clouds, item-rarity particles, built-in particle culling + render distance. Direct feature overlap is modest, but the niche is owned — and the pack already de-duplicated this lane once (Effectual's cave dust disabled because Particular owns it; Aug 2026 Wave 0.5 review).
- [x] **Dependency passes (checked)**: requires **Fzzy Config** (Modrinth API verified, required) — **already in pack** (Wave 3 dep, Easy Magic's 8-dep list) → zero NEW deps. Not a blocker.
- [x] **Compatibility**: author FAQ confirms it coexists with other ambient-particle mods (Fancy Block Particles, Particle Core, Visuality, Effective) — no hard conflict expected with Particular/Effectual; this is a pack-doctrine call, not a technical one.
- [x] **Revisit trigger**: user explicitly wants Subtle Effects' specific unique effects back (fire sparks, dust clouds, glowstone/redstone motes, item-rarity particles, snoring Z, particle culling/render distance), or Particular Reforged is removed/deprecated.
- [x] No GUIDE.md changes; mod counts unchanged; no PDF regen.
...
- [x] **CBC Enchanced Shells [Create Big Cannons] — ADOPTED** (CurseForge `cbc-enchanced-shells-create-big-cannons`, arginou57, ~377K DLs). Adds 86 munition items to Create Big Cannons: 50 standard shells (10 types × 5 tiers Mk.I–V — tiered iron → cast iron → diamond → netherite scrap → netherite ingot), 5 powder charges (rising muzzle velocity), 4 doomsday shells (Nuclear Mk.I/II, H-Bomb, Mini H-Bomb — 240+ block radius), a Shell Holder container, and a rocket system (launchpad + cascade-loading breech). Requires **Create 6.0+ + Big Cannons 5.11.2** (both in pack) — **zero new deps**. Fit: deepens the Wave 5 artillery/colony-defense role with a clean Phase 2→3 tier ladder; steampunk/Create-native (matches the weapon-ceiling doctrine). Overlap with Advanced Technologies is minor (AT = cannon blocks/autocannons; this = munition tiers/nukes) — complementary, not duplicate. **Watch**: nukes are player-fired/opt-in (chill-clean) but 240+ radius is extreme — verify a config toggle to tame/disable doomsday shells at launch; keep the shipped ×3 explosive cap on standard shells.
- [x] **Cataclysmic Combat — ADOPTED** (CurseForge `cataclysmic-combat`, FaerMinded, ~8.0M DLs). Better Combat movesets for L_Ender's Cataclysm weapons (Guard/Bulwark gauntlets → fist, Incinerator → claymore 2H, Void/Infernal forges → hammer 2H, Annihilator → mace, Soul Render → glaive). Requires **Better Combat + L_Ender's Cataclysm** (both in pack) — **zero new deps**. Fit: pure integration glue — makes two existing Wave 5 mods interoperate (satisfies the integration rule). No content duplication. Modrinth listing is Forge-only/outdated (1.20.x); the NeoForge 1.21.1 build lives on CurseForge. Author is a well-known Better Combat addon maintainer (Polyarmory, 20.1M total DLs).
- [x] **Better Swords Trims — SKIP** (CurseForge `better-swords-trims`, Valenn_Ro, ~966K DLs; Modrinth 2.0.0 NeoForge 1.21.1, Jul 20 2026, zero deps). Adds a "Spike" smithing template granting swords/armor combat effects (quartz=XP, emerald=levitate, lapis=glow, weeping obsidian=AoE, redstone=slow, copper=durability). **Concerns**: (1) the effect set duplicates Simply Swords/Arsenal's passive-ability role — the pack's gear fantasy is unique drops + enchantability, not a smithing-trim upgrade axis; (2) it targets vanilla sword/armor tiers ("iron to netherite"), so it's an orphan next to the Simply Swords/Simply More/Arsenal melee layer — competes with, and loses to, the loot chase. **Revisit trigger**: user explicitly wants a functional smithing-trim gear-customization layer, or proof the Spike trim applies to Simply Swords/Arsenal weapons.
- [x] **Create Sabers — SKIP** (CurseForge `create-sabers`, Leroidesafk, ~4.7M DLs; CurseForge-only). Star Wars lightsabers crafted from Create materials (brass/zinc/copper), multiple colors, double sabers, ponder integration — primarily cosmetic with light bonuses. **Concerns**: (1) laser-sword aesthetic clashes with the steampunk/Create-native weapon ceiling (same doctrine that excludes TACZ/Scorched Guns); (2) energy-melee overlaps Mekanism: Weapons' Meka-Tana (the pack's designated Phase 3 energy melee); (3) brass/zinc/copper gating puts an energy weapon in Phase 2, ahead of the Phase 3 Mekanism energy lane (Progression Gate Rule). **Revisit trigger**: re-themed to a non-lightsaber Create energy blade, or re-gated to Phase 3 materials.
- [x] **GUIDE.md updated**: Wave 5 table +2 rows (Cataclysmic Combat after L_Ender's Cataclysm; CBC Enchanced Shells after Advanced Technologies) and two detail sections ("Cataclysmic Combat — Better Combat for Boss Weapons", "CBC Enchanced Shells — Tiered Munitions"). Mod Count Summary: Wave 5 `6/5/11 → 8/5/13`; Total `174/42/217 → 176/42/218` (mods +2, deps unchanged — both zero-new-dep).
- [ ] **Instance action (XMCL)** — add both jars to `mods/`: **Cataclysmic Combat** (NeoForge 1.21.1, CurseForge `cataclysmic-combat` — Modrinth listing is the old Forge/1.20.x line) and **CBC Enchanced Shells** (NeoForge 1.21.1, CurseForge `cbc-enchanced-shells-create-big-cannons`). [VERIFY] at next launch: no missing-dependency errors; Cataclysm weapons (gauntlets/Incinerator/forges/Annihilator/Soul Render) animate with Better Combat movesets; CBC Enhanced Shells munitions appear in the CBC creative tab; check config for a doomsday-shell toggle and tame/disable nukes if too destructive for colony defense.
- [x] DRIFTWOOD-GUIDE.pdf regenerated
...
- [x] **Pinned 1.0.13+1.21.1+neoforge** (`zfastnoise-1.0.13+1.21.1+neoforge.jar`, 39.5 KB, Jul 8 2026, release, 64K DL on build; project 5.7M DL, MPL-2.0, updated Aug 17 2026 — very active). **Zero dependencies** (Modrinth API verified).
- [x] **Swap rationale — NoisiumForked is archived** (May 30 2026) and its own README recommends Fast Noise ("Noisium will be discontinued starting 26.3 as FastNoise exists as a better and more optimized version of the same. I recommend using FastNoise for all recent and upcoming versions."). Fast Noise declares Noisium/NoisiumForked/Moonrise incompatible (Modrinth-enforced) — cannot coexist; clean 1:1 swap. Same class of change already proven in this pack (NoisiumForked ran alongside Terralith/Tectonic/Incendium without issue; C2ME was skipped for seed non-determinism risk — Fast Noise keeps vanilla parity by only swapping storage paths). Author benchmarks: ~10–18% overworld worldgen, much higher Nether/End.
- [x] **Doc gap fixed**: NoisiumForked's adoption (Jul 2026 swap) was never reflected in GUIDE.md — no table row, not counted. GUIDE.md updated: Wave 0 Performance & Rendering table +Fast Noise row; Fast Noise note after the ModernFix config block. **Mod Count Summary** Wave 0 `8/10/19 → 9/10/20`; Total `174/43/218 → 175/43/219` (mods +1 — the newly documented mod; deps unchanged).
...
- [x] **Mod Count Summary updated** — Wave 2 37/7/45 → **36/7/44** (mods −1, deps unchanged); Total **175/43/219 → 174/43/218**. Changelog note appended.
- [x] **After First Launch note** — Dynamic Waters dropped from the first-world-load bullet (Terralith, Tectonic, and YUNG's remain).
- [ ] **Instance action (XMCL)** — remove `dynamicwaters-11.1.2.jar` from `mods/` if already added (zero-dep — no missing-dependency risk; vanilla rivers return, no new world needed). Supersedes the pending "add" action below. [VERIFY] at next launch: no missing-dependency errors; no further falling-through-world incidents around rivers.
...
- [x] **Root cause — Fancy Notify 1.0.0 bug**: the `onStartedPlaying` handler reads `musicSound.getLocation()` unconditionally, but in 1.21.1 `startPlaying()` returns early without setting `musicSound` when no music is selected (`getRandomMusic()` → null). **The Immersive Music Mod** (timm v1.1 — pack music overhaul that deliberately preserves "vanilla silence gaps") makes null-music the normal case, so the NPE fires at startup; would likely also crash in any music-less biome/context without TIM. Mixin chain confirms the crashing handler is `from mod fancynotify` (NeoForge-native — Sinytra Connector is a bystander).
- [x] **Decision — revert to Fancy Toasts 1.5.0** (the user-approved state before FN): FT has **no MusicManager mixin** (jar-verified — toast/GUI hooks only), so no music interaction; post-BadOptimizations fix (1.4.2+). Keeps the advancement-popup modernization. **Recipe toasts return to vanilla** (Dark Coffee GUI sprites) — the mod-based recipe-toast solution is deferred until FN is fixed.
...
- [x] **User decision (final) — swap to Fancy Notify**: remove **Advancement Plaques** (Wave 0 Inventory/UI; `AdvancementPlaques-1.21.1-neoforge-1.6.8.jar` instance-verified) and replace with **Fancy Notify 1.0.0** (Bivrik; CurseForge `fancy-notify` file 8679938 → `fancynotify-1.0.0-neoforge-1.21.1.jar`, release, Aug 18 2026; client-only, MIT, zero deps). Path this session: Advancement Plaques → Fancy Toasts → **Fancy Notify**. FT (Modrinth `eUziWqPC`, 1.5.0) was the initial pick for advancement popups, but FN is the author's all-in-one successor: it overhauls **every** vanilla toast — advancements, recipes, systems, music, biomes, weather, player logins, screenshots — with unique sprites and per-type filters, covering the advancement surface AND the recipe-toast surface in one mod.
- [x] **Jar-verified**: FN `fancynotify.mixins.json` hooks `RecipeToastMixin` (the "New Recipes Unlocked" toast), `SystemToastMixin`, `MusicManagerMixin`, `ScreenshotMixin`, `ClientPacketListenerMixin` (biome/weather/login), `ToastComponentMixin`, `ClientAdvancementsMixin` + `AdvancementToastMixinAccessor` — vs FT 1.4.7/1.5.0, whose mixins showed **no `RecipeToast` hook** (advancements only). Lang file confirms the per-type filter screen (Advancements/Recipes/Musics/Screenshots/Systems/Weathers/Biomes/Player Logins) + settings (transparency, width, anchor, List/Line orientation, animation, max amount, UI particles).
- [x] **Config note — Biomes notifications OFF**: FN's biome toasts would duplicate **Traveler's Titles** (biome/dimension entry titles; no-duplication rule). Disable at Mods screen → Fancy Notify → Filters → Biomes. Music toasts are a deliberate complement to The Immersive Music Mod (now-playing title/artist).
...
- [x] **GUIDE.md updated**: Wave 0.5 Visual & Client QoL table row (−Advancement Plaques −Fancy Toasts +Fancy Notify), What-to-Expect bullets, Game Rules note (also drops the stale "quest book" reference — FTB Quests removed Aug 24). Mod Count Summary **unchanged**: 1:1 mod swap (175 mods), Iceberg dep stays (48 deps).
- [ ] **Instance action (XMCL)** — remove `AdvancementPlaques-1.21.1-neoforge-1.6.8.jar` from `mods/`; add `fancynotify-1.0.0-neoforge-1.21.1.jar` (CurseForge download). **Keep Iceberg** (Item Borders). [VERIFY] at next launch: recipe/advancement/system toasts render Fancy Notify style (Mods screen → Fancy Notify → Filters: **Biomes OFF**); no missing-dependency errors; no double toasts.
- [x] DRIFTWOOD-GUIDE.pdf regenerated

### Recipe Toasts — Solved via Fancy Notify (Aug 25, 2026)

**Resolution**: the vanilla **"New Recipes Unlocked" toast** (`RecipeToast`) is now owned by **Fancy Notify** (jar-verified `RecipeToastMixin`). The original goal — control the recipe-toast surface — is met by the swap above: restyled with FN's unique recipe sprite, and **filtration** (Mods screen → Fancy Notify → Filters → Recipes) can hide it entirely if desired. No KubeJS script needed.

**Abandoned alternatives (verified, on record)**:
- **KubeJS `/recipe give @p *` login script** — moot: FN filtration replaces the need to unlock-all recipes.
- **Toast Control** (Shadows-of-Fire) — documented incompatible with the Fancy mods (toast-renderer conflict).
- **Restyle-only resource pack** — Dark Coffee GUI still styles vanilla toast sprites for toast types FN leaves to the vanilla system (FN compat note: unregistered toasts fall back to vanilla), but the main surfaces (recipes/advancements/systems) now use FN's own sprites.
- **Manual `/recipe give @p *`** — still valid as a one-off if toasts are ever wanted fully gone (needs temporary cheats; unlocks persist per world).

**Watch items**: FN is **v1.0.0, 7 days old, CurseForge-only** (Modrinth link 404s), 218 total DLs at adoption. Rollback path: re-add Advancement Plaques (or FT 1.4.7 — README's 1.21.1 pin) in one edit; nothing depends on FN. Re-verify + re-pin when the author ships a 1.0.x / Modrinth release.
...
- [x] **Fit check passed**: no content duplication — pack has zero salt/cheese/cinnamon/peanut/asparagus/sweet potato/chili/juice coverage (GUIDE.md grep-verified); More Delight (same author) recipes are distinct and designed to coexist. Integration: FD cooking pot/cutting board hub, Spice of Life variety, EMI/AE2, Create: Central Kitchen automatable stations (Cask/Juicer). Chill-clean; worldgen (wild crops, cinnamon tree, salt ore) benign. **Zero new deps** — requires Farmer's Delight only (in pack).
- [x] **User decision — SKIP for now**: mod is beta / "still in major development" (README banner); latest 1.21.1 NeoForge build `0.1.4-neoforge` (RTRYPIJp, Jan 10 2026, 264K DLs, `version_type: beta`) is 7.5 months old — slow maintenance cadence; not worth adding pre-stable.
- [x] **Revisit trigger**: a non-beta 1.21.1 release on Modrinth (any `version_type` ≠ beta). No GUIDE.md / Mod Count / PDF changes (no adoption).
### Mindful Darkness → Dark Coffee GUI Full Swap (Aug 2026)

- [x] **User decision — full swap**: remove **Mindful Darkness** (GUI dark-mode mod) and replace it with the **Dark Coffee GUI** resource pack (`v2.2.1`, 2026-07-03 — 1.21.1 verified via Modrinth API; client-side, zero deps, All Rights Reserved). Companion **Modded Coffee GUI** `v1.15.2` (2026-08-20, same author) added as the mod-compat layer. Supersedes the earlier Aug 2026 HUD-exemption config work — the two pending "copy `config/mindfuldarkness-client.toml`" instance actions are moot.
- [x] **Modern UI compatibility verified (clean)**: Modern UI's documented conflicts are tooltip/text-engine mods (Legendary Tooltips, Caxton, FeyTweaks, Emojiful, ImmediatelyFast, etc.) — not GUI-texture resource packs; the instance already proves the surface (Modern UI + Faithful 32x GUI textures). No shader-pipeline fight (unlike the old Dark Mode Everywhere watch). Pack text recolor follows JSON font definitions, which Modern UI honors; **NOSHADERS variant** is the documented fallback if text looks off under Iris. Tooltips unaffected — Obscure Tooltips owns the surface (Modern UI tooltips off), and the pack's custom tooltip frame is a 1.21.2+ feature (N/A on 1.21.1).
- [x] **Coverage tradeoff documented**: Modded Coffee GUI covers EMI, Create, Farmer's Delight, Waystones, Relics, Curios, Supplementaries, AppleSkin, Overflowing Bars, Spell Engine, Immersive Aircraft, Immersive Overlays, Sophisticated Storage/Backpacks (partial — scrollbar/search stay grey); **not covered (stay bright vanilla)**: Mekanism, AE2, MineColonies, Apotheosis, dimension mods — Mindful Darkness darkened every GUI uniformly; that coverage is the loss.
- [x] **Load order**: Dark Coffee GUI + Modded Coffee GUI placed at the **top** of the Resource Pack Load Order (above Improved Create 32x / Improved AE2 / Faithful 32x base) so the coffee GUI wins the GUI surface pack-wide; the 32x mod packs only overlap the GUI portions — item/block textures unaffected. Fallback documented: move the coffee packs below Improved Create/AE2 if the 32x mod GUIs are preferred.
- [x] GUIDE.md updated: Wave 0.5 Visual & Client QoL table row removed (−Mindful Darkness), Texture Packs table +2 rows and bullets (+Dark Coffee GUI +Modded Coffee GUI), Resource Pack Load Order renumbered (22 → 24 entries), Configuration — Wave 0.5 Mindful Darkness block removed, What-to-Expect bullet rewritten, Modern UI / tooltip-surface note updated, Mod Count Summary (Wave 0 8/12/20 → **7/12/19**; Total **176/48/225 → 175/48/224** — mods −1, deps unchanged: Puzzles Lib stays for Leave My Bars Alone / Hang Glider / Diagonal Fences; +2 resource packs → 24).
- [x] `config/mindfuldarkness-client.toml` deleted (override moot; historical config entries — Full HUD Exempted, Loot Journal Panels Exempted — superseded).
- [ ] **Instance action (XMCL)** — remove `mindfuldarkness*.jar` from `mods/` (Puzzles Lib stays); add `CoffeeGUI_1.20.2-26.2_release_v2.2.1.zip` + `ModdedCoffeeGUI_Release_v1.15.2.zip` to the instance's `resourcepacks/`, enable both at the **top** of pack order (Modded Coffee GUI directly above Dark Coffee GUI); delete the instance's `config/mindfuldarkness-client.toml` if present. [VERIFY] at next launch: vanilla + EMI/Create/Waystones screens render dark-coffee; Mekanism/AE2/MineColonies screens are bright vanilla (expected — documented tradeoff); no double-darkening; Modern UI Action Center unaffected; HUD/toasts styled by the pack (HUD is no longer bright-exempt — that was MD's tuned override).
- [x] DRIFTWOOD-GUIDE.pdf regenerated
...
- [x] **Fix**: Changed `generate_snow_ice = false` in `config/sereneseasons/seasons.toml` to disable seasonal water freezing and snow accumulation.
...
- [x] **Mod Count Summary updated** — Wave 2 38/7/46 → **37/7/45** (mods −1, deps unchanged); Total **182/49/231 → 181/49/230**. Changelog note appended.
- [ ] **Instance action (XMCL)** — remove `The Great Outdoors 1.21.1Neoforge V1.2.1.jar` from `mods/` if already added. [VERIFY] at next launch: no missing-dependency errors (zero-dep mod; GeckoLib unaffected).
- [x] STATUS adoption entry (below) annotated REMOVED.
- [x] DRIFTWOOD-GUIDE.pdf regenerated
...
- [x] **Create Deco — ADOPTED (Aug 23, 2026)**. Pinned **2.1.3** (`createdeco-2.1.3.jar`, Apr 18 2026, 1.31M DL on build; 41.8M DL project; zero new deps — Create only). Added to Wave 6 — Building table + new "#### Create Deco" detail section. Mod Count Summary: Wave 6 11/3/14 → **12/3/15**; Total **186/50/236 → 187/50/237** (mods +1, deps unchanged).
- [x] **Create Aircraft package — ADOPTED (Aug 23, 2026)**. Three items adopted as one Create-native integration:
  - **Immersive Aircraft bumped 1.1.5 → 1.4.6** (Wave 2 Mobility row; current 1.21.1 line, file 8158291, May 28 2026 — 1:1 version bump, no count change; matches the 3.0 packs' tested IA 1.4.x target).
  - **Create Immersive Aircrafts resource pack 3.0** (nicky__21, 9.5M DL) — Texture Packs table row + blurb + load order #21; retextures IA items/vehicles to Create style.
  - **Create Immersive Aircrafts - Recipes data pack 3.0** + **Create Man of Many Planes datapack 1.1** (MoMP: Fugimii, 570K DL) — zips downloaded and tracked in `datapacks/` (verified: pack_format 48, recipe data present); README Contents rows added; First Launch step updated; IA + MoMP guide sections note the Create-machine recipes. Recipe overrides = intentional design (Create-native aircraft progression); zero new mod deps.
- [ ] **Instance action (XMCL)** — add `createdeco-2.1.3.jar` to `mods/`; replace `immersive_aircraft-1.1.5+1.21.1-neoforge.jar` with `immersive_aircraft-1.4.6+1.21.1-neoforge.jar`; enable **Create Immersive Aircrafts** in the Resource Packs list; copy `Create Immersive Aircrafts Datapack 1.21.1 - 3.0.zip` + `Create-Man-of-Many-Planes-1.1.zip` into `saves/<world>/datapacks/` (or `%APPDATA%\.minecraft\datapacks\` for all worlds). [VERIFY] at next launch: Deco blocks craftable + shipping containers store items; IA 1.4.6 loads with MoMP 0.2.1 + Immersive Machinery 0.2.0; EMI shows Create-material aircraft recipes; RP textures applied on IA vehicles; no missing-dependency errors.
### Create Threaded Trains Removed (Aug 2026)

- [x] **User decision: remove Create: Threaded Trains** — the mod's own page documents that addons modifying Create's train code can freeze/crash ("works for some modpacks, not others"); with ~15 Create addons in the pack (incl. Steam 'n' Rails — the heaviest rail-code mod), the 1.0.0 thread-offload was the riskiest perf mod in the stack. Create Better FPS (client rendering) stays — different bottleneck, unaffected.
- [x] GUIDE.md updated: Wave 1 Create addons table row removed; Create Train Ecosystem table (six → five mods, performance step dropped); Mod Count Summary (Wave 1 22/5/27 → **21/5/26**; Total **187/50/237 → 186/50/236** — mods −1, deps unchanged).
- [ ] **Instance action (XMCL)** — remove `createthreadedtrains-neoforge-1.21.1-1.0.0.jar` from `mods/` (if already added). [VERIFY] at next launch: no missing-dependency errors; rail-network tick performance acceptable with Create's native pathfinding (Create Better FPS + ColorWheel still cover rendering).
