...
- [ ] **Instance action (XMCL)** — add `super_resolution-neoforge-1.21..1.21.1-0.9.1-alpha.1+opengl.jar` (client-only), `dynamicwaters-11.1.2.jar`, `The Great Outdoors 1.21.1Neoforge V1.2.1.jar` to `mods/`. [VERIFY] at next launch (fresh world for Dynamic Waters): no missing-dependency errors (all three zero-dep); SR upscaling engages on this GPU (alpha watch — try ratio >1.0 for FPS, verify no Iris/Sodium rendering artifacts); rivers flow with visible current + Create water wheels turn; Great Outdoors mobs spawn sane, Geyser Fields biome generates.
