...
5. **PDF Output** — The guide + modlist is compiled to a single PDF via Typst (v0.15.1) using `./generate-pdf.bat`. You MUST always verify the Typst version before writing ANY markup: run `typst --version` and confirm the output shows `Typst v0.15.1` (or later). If the version is anything else, do NOT write Typst code — ask the user to confirm first. All markup MUST use 0.15.1+ syntax only (no 0.14 or earlier patterns). Key rules: `#path()` does NOT accept `stroke:` as a named argument; `#circle()` does NOT take a position tuple as first argument; use `#scale()` to resize images to a percentage of original dimensions (NOT `width:` or `height:` which are relative to page size); use `#place(alignment, dx:, dy:, content)` for absolute positioning; use `#curve(components, stroke:, fill:)` for lines and curves; use `#stack(dir: ltr/ttb, spacing:, ...)` for flow layout of diagram elements. `#rect()`, `#circle()`, and `#line()` all accept `stroke:` and `fill:` as named arguments.
