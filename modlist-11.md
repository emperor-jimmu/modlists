...
- This subsection decides whether failure should remain vanilla's clean reload loop or become part of the narrative texture of the run.
- The best answer for `Elder Wilds` is the one that makes defeat feel meaningful without breaking momentum or making players hate experimentation.

### Options

- Emergent defeat route: `Shadow of Skyrim - Nemesis and Alternative Death System` - Nexus: <https://www.nexusmods.com/skyrimspecialedition/mods/65136>
- Vanilla route: keep death and reload as the default expectation.
- Hard-punishment route: only consider harsher alternatives if the list explicitly wants failure itself to become a dominant progression mechanic.

### Recommendation

- Keep `Shadow of Skyrim` as the leading comparison route here.
- Its strongest argument is not raw punishment but continuity: defeat becomes a story beat, gear-loss scare, recovery quest, and revenge arc instead of an immersion-breaking reload.
- Community signal for it is unusually strong, and the mod is clearly good at creating memorable emergent stories, but it is also a very authored layer and should be judged on tone-fit rather than novelty alone.
- If the extra scripting, debuffs, or nemesis theatrics feel too gamey for the final list, default death-and-reload is still the safer baseline.

### Risks & Compatibility

- `Shadow of Skyrim` changes the emotional meaning of death much more than a simple penalty mod would.
- Gear theft, relocation, and debuff recovery can be excellent for story-driven runs but frustrating if the rest of the list is already highly punishing.
- This subsection can easily overshoot if defeat, survival pressure, and harsher encounters all peak at the same time.

### Acceptance Criteria

- Failure feels consequential without making the player want to disable the whole system after a few bad runs.
- If `Shadow of Skyrim` is chosen, nemesis stories feel like part of the wilderness-survival fantasy rather than a disconnected gimmick.
- Defeat handling preserves momentum better than vanilla reload spam while staying compatible with the rest of the list's tone.
